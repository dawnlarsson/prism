...
// get_self_exe_path
// This is called from prism.c's get_self_exe_path() which has platform #if chains.
// We provide a helper so the existing readlink("/proc/self/exe") call returns -1,
// Prism POSIX shim layer for native Windows (MSVC cl.exe)
...
// This is called from prism.c's get_self_exe_path() which has platform #if chains.
// We provide a helper so the existing readlink("/proc/self/exe") call returns -1,
// and prism.c can add a _WIN32 branch that calls GetModuleFileNameA directly.
// (This is handled in the minimal prism.c patch below.)
