...
**Input contract: Prism is NOT a C compiler front-end.** Prism's job is to take *well-formed* C, add safety transforms, and emit equivalent well-formed C; it is the back-end C compiler's job (clang, gcc, MSVC) to diagnose ISO grammar violations like `int x,,y;`, `enum E { A=1 B=2 }`, `''`, `0x` with no hex digits, `1e+` with no exponent, attributes on number literals, `int (p[3])(int)`, `i ?? 1`, etc. Prism's tokenizer is intentionally permissive: noise, malformed declarators, illegal operator chains, and similar lexical/grammatical garbage are passed through verbatim so the back-end compiler can produce its own (better-localized) diagnostics. **Prism does** raise hard errors for: (a) invariants its own transforms depend on (defer/orelse/zeroinit/bounds-check semantics, scope leakage, control-flow safety, two-pass invariant violations), (b) constructs it cannot translate soundly (e.g. `const` + unavoidable memset, computed goto over zeroinit declarations, `register` + VLA), and (c) a small set of clearly-stranded statements that would otherwise misroute Prism's own state machine (file-scope `return`/`break`/`continue`/`if`/`for`/`while`/`do`/`switch`/`else`/`case`/`default`/`goto`, stray `case`/`default` outside any active switch).

**Standards compatibility:** Prism accepts C99, C11, and C23 input and emits standard C compatible with GCC, Clang, and MSVC. All standard C type specifiers, qualifiers, storage classes, attributes, and control-flow constructs are recognized and passed through correctly. C23 features including `typeof_unqual`, `constexpr`, `auto` type inference, `_BitInt(N)`, `[[...]]` attributes, `alignas`/`alignof`, `static_assert`, fixed-underlying-type enums (`enum E : int { ... }`), labeled declarations, and if/switch initializers are supported. GCC extensions `__typeof_unqual__` and `__typeof_unqual` are recognized as `PPARSE_TT_TYPE | PPARSE_TT_TYPEOF` and handled identically to C23 `typeof_unqual`. C23 TS 18661 extended float types (`_Float16`, `_Float32`, `_Float64`, `_Float128`, `_Float32x`, `_Float64x`, `_Float128x`) and decimal float types (`_Decimal32`, `_Decimal64`, `_Decimal128`) are registered as `PPARSE_TT_TYPE` keywords, ensuring zero-initialization and declaration detection work correctly. GCC-specific float types (`__float128`, `__float80`, `__fp16`, `__bf16`) are similarly registered. Do note Prism IS NOT officially certified in any way.

Note: Prism is thoroughly empirically tested (self-hosting and the full harness above) but **is not formally verified.** It is designed to compile standard-compliant code, but **may not catch every obscure constraint violation** defined by the ISO C standard.

The transpiler operates in two passes:

1. **Pass 1:** Full semantic analysis over all tokens at all depths. Builds immutable data structures (scope tree, symbol table, shadow table, per-function CFG arrays), annotates tokens (`ann` bits), and records per-declaration emit recipes. Raises all errors. No output is emitted.
2. **Pass 2:** Code generation. Reads Pass 1's immutable artifacts and trusts its annotations/recipes. Emits transformed C. No type table mutations, no safety checks, no speculative token walking. If Pass 2 starts, it runs to completion.

**Key invariant:** Every semantic error is raised before Pass 2 emits its first byte.

**Debug assert shells:** Semantic rejects that Phase 1 provably owns are retained in Pass 2 only under `#ifdef PRISM_DEBUG`: in release builds Pass 2 cannot disagree with Phase 1, and in debug builds any Phase 1 coverage hole trips the assert. The full suite is run against both builds; a debug-only failure means "hoist the missing case into Phase 1", never "keep the Pass 2 check live".
...
`PPARSE_TT_TYPE`, `PPARSE_TT_QUALIFIER`, `PPARSE_TT_SUE` (struct/union/enum), `PPARSE_TT_SKIP_DECL`, `PPARSE_TT_ATTR`, `PPARSE_TT_ASSIGN`, `PPARSE_TT_MEMBER`, `PPARSE_TT_LOOP`, `PPARSE_TT_STORAGE`, `PPARSE_TT_ASM`, `PPARSE_TT_INLINE`, `PPARSE_TT_NORETURN_FN`, `PPARSE_TT_SPECIAL_FN`, `PPARSE_TT_CONST`, `PPARSE_TT_RETURN`, `PPARSE_TT_BREAK`, `PPARSE_TT_CONTINUE`, `PPARSE_TT_GOTO`, `PPARSE_TT_CASE`, `PPARSE_TT_DEFAULT`, `PPARSE_TT_DEFER`, `PPARSE_TT_GENERIC`, `PPARSE_TT_SWITCH`, `PPARSE_TT_IF`, `PPARSE_TT_TYPEDEF`, `PPARSE_TT_VOLATILE`, `PPARSE_TT_REGISTER`, `PPARSE_TT_TYPEOF`, `PPARSE_TT_BITINT`, `PPARSE_TT_ALIGNAS`, `PPARSE_TT_ORELSE`, `PPARSE_TT_STRUCTURAL`.

GNU extension `__auto_type` is tagged `PPARSE_TT_TYPE | PPARSE_TT_TYPEOF` so that declaration detection and goto-over-decl checking treat it identically to `typeof`/`__typeof__`. The `typeof_unqual` family (`typeof_unqual`, `__typeof_unqual__`, `__typeof_unqual`) is tagged `PPARSE_TT_TYPE | PPARSE_TT_TYPEOF`; `pparse_type_specifier` detects the unqual variant via `tok->len >= 13` (all three lengths (13, 15, 17) exceed the longest non-unqual typeof variant `__auto_type` at 11) and skips the inner qualifier propagation scan, since `typeof_unqual` strips qualifiers by definition.

Some C23 or extension spellings are contextual for Prism's supported C99/C11 inputs: `alignas`, `alignof`, `asm`, `bool`, `constexpr`, `noreturn`, `offsetof`, `static_assert`, `thread_local`, `typeof`, and `typeof_unqual` may still be ordinary identifiers when a backend accepts them as such. In declarator-name position, Prism treats those tokenized keywords as valid names and stops the type-specifier parser before consuming them when they follow an established type and are followed by a declaration boundary. Typedef lookup and struct/union tag registration also accept these soft-keyword tokens, so `typedef struct { int a; } bool; bool x;`, `typedef struct { int b; } alignas; alignas x;`, and `struct alignas { int a; }; struct alignas y;` preserve aggregate metadata. A real typedef entry for a soft keyword wins over built-in soft-keyword tags in type-name position, while non-typedef spellings still keep true specifier forms such as `constexpr int x;`, `alignas(16) int x;`, and `typeof(expr) x;` intact. Soft-keyword spellings are also accepted as labels/goto targets, for example `goto alignas; alignas:`.

Prism extension spellings are contextual in C declaration positions too. `defer`, `orelse`, and `raw` can be typedef names, variable names, function names, parameters, or struct/union/enum tags when the surrounding C grammar makes them identifiers. In particular, struct/union/enum type specifiers are not considered expression-ending tokens for `orelse` disambiguation, including anonymous type bodies, named tags, fixed-underlying-type enums, and GNU attribute noise between the type body/tag and declarator name. Backend-valid declarations such as `typedef struct { int a; } orelse;`, `struct { int a; } orelse;`, `enum E : int { A }; enum E orelse;`, `struct { int a; } __attribute__((unused)) orelse;`, `int orelse(void); orelse();`, and `int f(int orelse);` are emitted as ordinary C declarations or calls rather than being parsed as Prism `orelse` or `defer` syntax. Likewise, a `)` that closes a C type-name cast does not force a following identifier named `orelse` to be parsed as the Prism operator, so `(void)orelse;` remains an ordinary cast expression when `orelse` is a variable.

`orelse` is also contextual in label and array-dimension grammar. `orelse:` and `goto orelse;` are C labels/targets, not Prism fallback syntax. GNU local-label declarations and computed-goto label addresses preserve Prism keyword spellings as identifiers too, such as `__label__ orelse, defer, raw;`, `&&orelse`, and parenthesized/static-initializer forms like `(&&orelse)` or `static void *p = &&orelse;`. Inside `[...]`, Prism only classifies `orelse` as the array-dimension fallback operator when it follows an expression-ending token that can serve as the left-hand side; an enum constant or variable named `orelse` at the start of the dimension remains an ordinary identifier.

### Taint graph

The tokenizer builds per-function taint flags (`has_setjmp`, `has_vfork`, `has_asm_goto`) stored on the opening `{` token's tag bits (`PPARSE_TT_SPECIAL_FN`, `PPARSE_TT_NORETURN_FN`, `PPARSE_TT_ASM`). These remain on the token and are read directly via `func_meta[idx].body_open->tag`: they are **not** transferred to `FuncMeta` fields. The `PPARSE_TT_ASM` taint fires only for `asm goto`: the tokenizer scans between the `asm` keyword and `(` for a `PPARSE_TT_GOTO` token. Regular `asm` (volatile, inline) is safe and does not taint.

**Wrapper taint mirrors direct-body scan:** When propagating taint from a wrapper function (`void wrap(...) { callee(...); }`) to its callers, the propagation rule must match the rule used by the direct-body scan, otherwise direct vs wrapped calls behave inconsistently. The direct-body scan only tags a body with `PPARSE_TT_NORETURN_FN` when the body literally contains the `vfork` token (the `PPARSE_TT_SPECIAL_FN` branch hardcodes the `'v' && len==5` check); bare `PPARSE_TT_NORETURN_FN` callees (`exit`, `_Exit`, `abort`, `quick_exit`, `thrd_exit`) never taint the body directly: they only produce a per-call-site warning in Pass 2. The `wrapper_taint[]` initialization mirrors this: only `PPARSE_TT_SPECIAL_FN` callees populate `wrapper_taint[i]` (vfork wrapper → `PPARSE_TT_NORETURN_FN`, setjmp/longjmp/pthread_exit wrapper → `PPARSE_TT_SPECIAL_FN`). A bare-`PPARSE_TT_NORETURN_FN` callee leaves `wrapper_taint[i] = 0`. Without this mirror, `void die(int c) { exit(c); }` would propagate `PPARSE_TT_NORETURN_FN` onto every caller's body, hard-rejecting any defer in those callers with the misleading message "defer cannot be used in functions that call vfork()", while a direct `exit(c)` only produces a warning at the call site.

**Noreturn attribute scanning:** After tokenization, the tokenizer scans all tokens for noreturn annotations and tags every occurrence of the function name with `PPARSE_TT_NORETURN_FN`. Four detection modes are used (one keyword specifier + three attribute syntaxes). When the forward scan from the attribute position fails to find a function name (e.g., post-declarator attributes like `void my_die(void) __attribute__((noreturn));`), a backward scan through the token pool looks for an `ident(` pattern, stopping at `;`, `{`, `}`:

- **`_Noreturn` / `noreturn` keyword:** Bare keyword check (`pparse_equal(t, "_Noreturn") || pparse_equal(t, "noreturn")`).
- **C23 `[[noreturn]]` / `[[_Noreturn]]` / `[[__noreturn__]]`:** When a `[` token with `PPARSE_TF_C23_ATTR` is found, all `PPARSE_TK_IDENT` tokens between `[[` and `]]` are scanned for `noreturn`, `_Noreturn`, or `__noreturn__`. This handles namespaced forms (`[[gnu::noreturn]]`, `[[gnu::__noreturn__]]`) because the loop skips non-matching identifiers.
...
| Flag | Bit | Meaning |
|---|---|---|
| `P1_IS_TYPEDEF` | 0 | PParseToken resolves to a real typedef at this position (baked by Phase 1 annotation pass) |
| `P1_SCOPE_LOOP` | 1 | This `{` opens a loop body |
| `P1_SCOPE_SWITCH` | 2 | This `{` opens a switch body |
| `P1_HAS_ENTRY` | 3 | PParseToken has any typedef-table entry (typedef, enum constant, shadow, or VLA variable) |
...
Ancestor check (`pparse_scope_is_ancestor_or_self`): O(depth) walk up `parent_id` chain, typically < 10 hops.
...
The `scope_stack[]` is distinct from `pparse_scope_tree[]` (Phase 1A). The scope tree is a flat immutable array indexed by `scope_id`; the scope stack is a mutable LIFO stack driven by `{`/`}` during Pass 2 emission.

`SCOPE_CTRL_PAREN` / `SCOPE_FOR_PAREN` nodes created at `(` after `while` / `for` / `switch` carry `is_loop` / `is_switch` (from the keyword, not from a following `{`). `defer_walk` uses this so `break` / `continue` inside a GNU statement expression in a **condition** (e.g. `while (({ if (1) break; 1; }))`) stops at the control paren and does not run outer defers as if exiting the whole function. Before this, only `SCOPE_BLOCK` carried those flags and the loop body block did not exist yet.

#### Per-function metadata

```
FuncMeta {
    body_open              : PParseToken*
    ret_type_start         : PParseToken*
    ret_type_end           : PParseToken*
...
**Taint flags:** `has_setjmp`/`has_vfork`/`has_asm_goto` are NOT stored in `FuncMeta`. They are tag bits (`PPARSE_TT_SPECIAL_FN`, `PPARSE_TT_NORETURN_FN`, `PPARSE_TT_ASM`) on the function body's opening `{` token, set by the tokenizer during taint-graph construction. Checked at defer-allocation time via `func_meta[idx].body_open->tag`.
...
**Decl recipes:** `decl.shape` packs `classify_decl_shape` results as `P1DS_*` bits (`EFF_VLA`, `AGG`, `UNION`, `FUNC`); `decl.zero_kind` is the pre-computed zero-init emit recipe (`P1Z_NONE` / `P1Z_SCALAR` = ` = 0` / `P1Z_AGG` = ` = {0}` / `P1Z_MEMSET` = delayed memset). Both are recorded by `p1_record_local_decl` for every ordinary local. Pass 2's `process_declarators` looks the entry up via `p2_lookup_decl_entry` (keyed by the declarator name's token index) and emits from the recipe instead of re-classifying; the only demotion Pass 2 may apply is `P1Z_MEMSET` → `P1Z_AGG` inside `for`/`if`/`switch` init statements, where a delayed memset cannot be placed. Live `classify_decl_shape` remains as fallback for declarations with no entry (file scope, function types, aggregate members); a `PRISM_DEBUG` assert flags any in-function local that misses its recipe.
...
|---|---|---|
| `PParseToken.ann` | Inline in token pool | `pparse_tokenizer_teardown(true)` (frees token pool) |
| `pparse_scope_tree[]` | `PPARSE_ARENA_ENSURE_CAP` | `pparse_tokenizer_teardown(false)` |
| `func_meta[]` | `PPARSE_ARENA_ENSURE_CAP` | `pparse_tokenizer_teardown(false)` |
| `p1_entries[]` | `PPARSE_ARENA_ENSURE_CAP` | `pparse_tokenizer_teardown(false)` |
| `p1_shadow_entries[]` | `PPARSE_ARENA_ENSURE_CAP` | `pparse_tokenizer_teardown(false)` |
| `p1_shadow_map.buckets` | `pparse_arena_alloc` | `pparse_tokenizer_teardown(false)` |
| `typeof_vars[]` | `PPARSE_ARENA_ENSURE_CAP` (pparse_ctx, reused) | `pparse_tokenizer_teardown(false)` |
| `bracket_oe_ids[]` | `PPARSE_ARENA_ENSURE_CAP` (pparse_ctx, reused) | `pparse_tokenizer_teardown(false)` |

All arena-allocated structures are automatically reclaimed by `prism_reset()` → `pparse_tokenizer_teardown(false)` on any `pparse_error_tok()` that `longjmp`s out. `pparse_arena_alloc_uninit` has an overflow guard: `size > SIZE_MAX - (PPARSE_ARENA_ALIGN - 1)` is rejected before the alignment addition, preventing wrap-around to a tiny allocation.
...
| `else {` | `is_conditional` |
| `({` | `is_stmt_expr` |
| `struct`/`union`/`enum` keyword (or tag name preceded by `PPARSE_TT_SUE`) `{` | `is_struct` |
| `) {` where `(` preceded by `__attribute__`/`PPARSE_TT_ATTR`: walk further back past balanced parens and attributes to find the real keyword | Inherits classification from the real keyword (`is_loop`, `is_switch`, etc., or `is_struct` when `PPARSE_TT_SUE` found) |
| Named struct with attributes: `struct __attribute__((packed)) Name {` | `is_struct` (look-behind skips balanced parens and `PPARSE_TT_ATTR` tokens) |
...
| `enum` keyword (or tag name preceded by enum `PPARSE_TT_SUE`) `{` | `is_struct` + `is_enum` |

**C23 attribute backward walk:** When walking backward to find `prev`, the loop skips `]]` C23 attributes by checking `pparse_pair(])->flags & PPARSE_TF_C23_ATTR` (the `PPARSE_TF_C23_ATTR` flag is set on the opening `[`, not the closing `]`). This ensures `void f(void) [[gnu::cold]] {` correctly finds `)` as `prev` and classifies it as `is_func_body`.
...
Walks all tokens at all depths. Registers every `typedef`, `enum` constant, and VLA tag into the typedef table using `pparse_typedef_add_entry`.

Each `PParseTypedefEntry` records:

- `name`, `len`: identifier text
- `scope_depth`: numeric depth
- `scope_open_idx`, `scope_close_idx`: token index range of the enclosing scope (set from `td_scope_open` / `pparse_td_scope_close` thread-locals, which are updated as Phase 1 enters/exits scopes)
- `token_index`: pool index of the declaration
- `is_vla`, `is_void`, `is_const`, `is_volatile`, `is_ptr`, `is_array`, `is_aggregate`, `has_volatile_member`, `is_atomic`: type property flags (`is_atomic` is set when the typedef's type specifier used `_Atomic(...)`: see `PPARSE_TDF_ATOMIC` / `pparse_typedef_apply_tdf_flags`)
- `is_shadow`, `is_enum_const`, `is_vla_var`, `is_struct_tag`: entry kind flags. `is_struct_tag` entries (`PPARSE_TDK_STRUCT_TAG`) are unconditionally registered for all struct/union tags with bodies, including clean structs with no VLA or volatile members. This ensures inner-scope struct redefinitions correctly shadow outer tags (C11 §6.2.1p4), preventing `pparse_tag_lookup` from falling through to stale outer-scope VLA/volatile metadata and causing false CFG verifier errors. Registration occurs at two sites: Phase 1A's `PPARSE_TT_SUE` handler (bare struct defs) and `pparse_typedef_declaration` (typedef-wrapped struct defs). Clean entries have `is_vla=false, has_volatile_member=false`. They do NOT receive `P1_IS_TYPEDEF` annotation, preserving `struct Foo` parsing (the tag is not an ordinary typedef name per ISO C11 §6.2.3). Tag name extraction in Phase 1D uses `pparse_skip_noise()` to skip GNU `__attribute__` and C23 `[[...]]` attributes between the struct/union keyword and the tag name, and skips `PPARSE_TT_QUALIFIER` tokens
- `is_func`: set when the typedef resolves to a function type, and also on `PPARSE_TDK_SHADOW` entries that represent block-scope function declarations. This suppresses zero-init memset on function types and lets `typeof(name)` distinguish a local function declaration from an ordinary local variable/parameter shadow. Typedef detection works for both `typedef int FuncType(int)` (parse_declarator returns `end=NULL`, check for `(` after name), `typedef int (FuncType)(int)` (parse_declarator sets `is_func_ptr=true` without `paren_pointer`: function type, not function pointer), and parenthesized function declarators such as `typedef int (*Getter(void))[10]` (`decl.is_func_decl`). **Chained typedef propagation:** `pparse_typedef_declaration` propagates `is_func` through typedef chains via `base_is_func`: when the base type specifier contains a function typedef (`pparse_is_func_typedef`), the derived typedef inherits `is_func = true` (guarded by `!decl.is_pointer && !decl.is_array && !decl.is_func_ptr`). Without this, `typedef func_t alias; alias f;` would lose the function-type property and Prism would emit `= {0}` or `memset` on a function symbol: a fatal constraint violation (ISO C11 §6.5.3.4p1 forbids `sizeof` on function types).
- `is_param`: set for function parameter shadow entries (registered by `p1_register_param_shadows`). Used by `pparse_array_size_is_vla` to distinguish VLA parameters (which decay to pointers, making `sizeof(param)` constant) from VLA locals.
...
The duplicate-detection guard in `pparse_typedef_add_entry` compares entry kind (`prev->is_struct_tag == (kind == PPARSE_TDK_STRUCT_TAG)`) in addition to scope depth and range: a `PPARSE_TDK_TYPEDEF` and `PPARSE_TDK_STRUCT_TAG` with the same name at the same scope are NOT duplicates (separate C11 namespaces).

Scope check for each candidate entry:

```
entry.token_index <= pparse_idx(tok) AND
pparse_idx(tok) >= entry.scope_open_idx AND
pparse_idx(tok) < entry.scope_close_idx
```
...
#### pparse_typedef_flags query

A single lookup returns a bitmask: `PPARSE_TDF_TYPEDEF`, `PPARSE_TDF_VLA`, `PPARSE_TDF_VOID`, `PPARSE_TDF_ENUM_CONST`, `PPARSE_TDF_CONST`, `PPARSE_TDF_PTR`, `PPARSE_TDF_ARRAY`, `PPARSE_TDF_AGGREGATE`, `PPARSE_TDF_VOLATILE`, `PPARSE_TDF_FUNC`, `PPARSE_TDF_PARAM`, `PPARSE_TDF_HAS_VOL_MEMBER`, `PPARSE_TDF_UNION`, `PPARSE_TDF_ATOMIC`. Convenience macros (`pparse_is_known_typedef`, `pparse_is_vla_typedef`, `pparse_is_volatile_typedef`, `pparse_has_volatile_member_typedef`, etc.) test individual bits.
...
For every variable declaration at every depth, if the declared name collides with a typedef, a `P1ShadowEntry` is recorded via `p1_register_shadow`. Shadows also create a `PPARSE_TDK_SHADOW` entry in the typedef table itself (with `is_shadow = true`, `scope_open_idx`, `scope_close_idx`), so that `pparse_typedef_lookup` returns the shadow entry when the token is within the shadow's scope range. Shadow registration triggers when the declared name matches any of: `PPARSE_TT_DEFER`, `PPARSE_TT_ORELSE` (keyword shadowing), `PPARSE_TT_NORETURN_FN`, `PPARSE_TT_SPECIAL_FN` (noreturn/setjmp function name shadowing), or an existing typedef/enum-constant/function-prototype entry in the typedef table (type shadowing). Block-scope function declarations (`decl.is_func_decl`) also register a `PPARSE_TDK_SHADOW` entry marked `is_func = true`; this preserves local prototype visibility for `typeof(local_function_name)` while still allowing ordinary object shadows to hide function/prototype names. **Volatile force-registration:** declarations whose type is volatile-qualified (directly via `PPARSE_TT_VOLATILE`, transitively via a `PPARSE_TDF_VOLATILE` / `PPARSE_TDF_HAS_VOL_MEMBER` typedef, or whose struct/union tag carries `has_volatile_member`) force a shadow entry even when no name-collision exists, so Pass 2's `emit_token_range_orelse` sees the volatile qualification via a normal `pparse_typedef_lookup` and rejects compound-literal fallbacks uniformly. This covers ordinary block-scope locals (main decl loop), function parameters (`p1_register_param_shadows`), and for-/if-/switch-init declarators (`p1_scan_init_shadows`).
...
| `P1K_DECL` | scope_id, token_index, `has_init`, `is_vla`, `has_raw`, `is_static_storage`, plus the `shape` / `zero_kind` emit recipe (§3.0) (allocated only for runtime declarations inside function bodies: struct/union body fields are skipped via `pparse_scope_tree[cur_sid].is_struct` because they are type members, not runtime objects, and must not block the computed/asm-goto "function contains zero-initialized declarations" gate) |
| `P1K_SWITCH` | scope_id, token_index (braced switches use Phase 1A scope_id; braceless switches use a synthetic scope_id beyond `pparse_scope_tree_count`) |
...
**File-scope control-statement rejection:** Phase 1D's main prescan loop hard-rejects control statements that appear at file scope (not inside any function body). This fires when `p1d_cur_func == -1`, `p1d_init_brace_depth == 0`, `at_stmt_start` is true, and the token is *not* a known typedef. Three classes are rejected: (a) `return`/`break`/`continue`/`case`/`default` (matched via `PPARSE_TT_RETURN | PPARSE_TT_BREAK | PPARSE_TT_CONTINUE | PPARSE_TT_CASE | PPARSE_TT_DEFAULT`); (b) `goto` (`PPARSE_TT_GOTO`); (c) `if`/`for`/`while`/`do`/`switch`/`else` (`PPARSE_TT_IF | PPARSE_TT_LOOP | PPARSE_TT_SWITCH` or `pparse_is_else_kw`). The check runs before any of the function-only Phase 1D handlers, so the offending token never reaches Pass 2's emission paths and the CFG verifier sees a structurally well-formed token stream. Rationale: ISO C grammar permits these statements only inside a `compound-statement` that is the body of a `function-definition` (C11 §6.8). Pre-fix, Prism let them pass through and leaned on the back-end C compiler to diagnose; this allowed downstream passes to misclassify the surrounding tokens (e.g., a stray `case` would synthesize a `P1K_CASE` entry whose `switch_scope_id` is meaningless).

**Stray case/default rejection (Pass 2A / `p1_verify_cfg`):** Inside a function, a `case` or `default` label whose `kase.switch_scope_id` is `0` is hard-rejected. Phase 1D records `switch_scope_id = 0` only when `p1d_switch_top == 0` at allocation time, i.e., the label appears outside any active braced or braceless switch. Real switch scopes always have `sid > 0` (file scope is `0`, but `case` at file scope is already caught by the file-scope check above). The error is "case/default label outside any switch statement". This catches patterns like `switch (x) do { } while (0); case 1: …` where the outer braceless switch's body has already been popped from `p1d_switch_stack` by the time the trailing `case` is encountered.

**Attribute control-flow rejection:** After `pparse_skip_noise()` advances past GNU `__attribute__((...))`, C23 `[[...]]`, or `_Pragma(...)` blocks, the prescan scans the skipped token range for control-flow keywords (`goto`, `return`, `break`, `continue`, `defer`) and `orelse`. If found, a hard error is emitted. Without this, a `goto` inside a statement expression in an attribute argument (e.g., `__attribute__((aligned( ({ goto L; 8; }) )))`) would be invisible to the CFG verifier: no `P1K_GOTO` entry would be created, and the jump-over-VLA safety check would not fire. Pass 2's `emit_range_ex` correctly routes `({...})` through `walk_balanced` → `emit_statements(EMIT_NORMAL)`, which handles defer cleanup at the goto site; however, Phase 1D's blindness would leave the static CFG graph incomplete. The `defer` and `orelse` checks use `pparse_typedef_lookup` to avoid rejecting variables named `defer` or `orelse`. The `orelse` check is also performed independently in the `!at_stmt_start` handler for mid-declarator and trailing attributes: when a balanced attribute group (GNU `__attribute__((...))` or C23 `[[...]]`) is encountered, the prescan scans its interior for `PPARSE_TT_ORELSE` tokens (with the same `pparse_typedef_lookup` shadow guard). Pass 2 emits C23 `[[...]]` interiors verbatim (`emit_token_range_nested(start, end, /*with_orelse=*/false)`: no `orelse`→ternary rewrite); valid sources never reach emission with Prism `orelse` inside an attribute argument.

**Init-statement zero-init via `= {0}`:** A delayed `__builtin_memset` cannot be inserted between an init-statement declaration and the controlling expression, so any declarator in a `for`/`if`/`switch` init-statement that *would otherwise* require memset (union, `_Atomic` aggregate, `typeof(...)`, `_Atomic(typeof(...))`, typedef thereof) is satisfied with `= {0}` (or `= 0` for typeof scalars / pointers) directly at the declarator. This is the de-facto universal zeroing form on gcc and clang: verified empirically with `-O0`/`-O2` against polluted stacks for unions up to 64 bytes. **For-init VLAs remain rejected**: ISO C99 §6.7.8p3 forbids initializer-list syntax on variable-length arrays, and a delayed `__builtin_memset` cannot land between the init clause and the body. **C23 if/switch-init VLAs are accepted, uninitialized**: the variable simply remains uninitialized (matching plain C semantics): the user can opt back into Prism zero-init by hoisting the declaration before the `if`/`switch`. The for-init vs. if/switch-init distinction is made by checking `scope_stack[depth-1].is_loop` on the `SCOPE_FOR_PAREN` (set on `for`-paren only; if/switch-init parens reuse `SCOPE_FOR_PAREN` for `at_stmt_start` purposes but have `is_loop == false`). The Phase 1D `p1_scan_init_shadows` takes a new `is_for_init` parameter; the for-init call site passes `true`, the if/switch-init call site passes `false`. Pass 2 `process_declarators` mirrors the gate via `real_for_init = in_for_init() && scope_stack[depth-1].is_loop`. Everything else flows through `init_stmt_brace = needs_memset && (in_for_init() || in_ctrl_paren()) && !effective_vla` → emit `= {0}` and skip the memset queue. **`const` interaction:** the otherwise-mandatory rejection of `const` + unavoidable-memset (UB per C11 §6.7.3p6) is also lifted in init-statement context because no memset is emitted (and brace-init of a `const` object is a normal initialization, not a const-modify).

**For-init / if-switch-init declarations:** `p1_scan_init_shadows` also allocates `P1K_DECL` entries for variables declared in C99 for-init clauses (`for (int i = 0; ...)`) and C23 if/switch initializers (`if (int x = f(); x > 0)`). The scope_id assigned to these entries is the **body scope** (the `{` following the `)` of the control statement), not the enclosing scope. This ensures the CFG verifier catches gotos that jump *into* the loop/if/switch body past the init declaration, while still allowing gotos that jump *over* the entire control statement (where the variable is no longer in scope). For braceless bodies (no `{`), the entry uses the enclosing scope's `scope_id` (cur_sid) with a `body_close_idx` field set to the braceless body's end position (from `pparse_skip_one_stmt`). The CFG verifier uses `body_close_idx` when non-zero to correctly bound the variable's lifetime, allowing gotos that jump *over* the entire statement while catching gotos that jump *into* the braceless body. Consecutive `raw` keywords (e.g. from macro expansion like `raw raw int arr[n]`) are handled by a `while` loop, matching the pattern in `try_zero_init_decl`. **For-init typedef routing:** When the first token after `(` (and any `raw` prefix) has the `PPARSE_TT_TYPEDEF` tag, `p1_scan_init_shadows` delegates to `pparse_typedef_declaration` with `td_scope_open`/`pparse_td_scope_close` set to the enclosing control statement's scope bounds. Enum constants within the typedef body are registered via `pparse_enum_constants`. Without this routing, `typedef` in for-init/if-init is silently dropped: the guard condition only checks `PPARSE_TT_TYPE|PPARSE_TT_QUALIFIER|PPARSE_TT_SUE|PPARSE_TT_TYPEOF|PPARSE_TT_BITINT` and `pparse_is_known_typedef()`, none of which match the `PPARSE_TT_TYPEDEF` keyword tag. The consequence is type system collapse: variables declared using the for-init typedef are not recognized as typed declarations, bypassing both zero-initialization and the CFG verifier's goto-over-VLA safety checks.

**Braceless-body declarations:** Declarations that constitute the body of a braceless control statement (e.g., `if (1) L: int x;` using C23 labeled declarations) receive `body_close_idx` from `pparse_skip_one_stmt` on the type token. Phase 1D detects this via `p1d_ctrl_pending`: when the flag is true at declaration time (labels don't reset it), the declaration is bounded to the braceless body's semicolon. Case/default labels DO reset `p1d_ctrl_pending` via `P1D_STMT_RESET()`: they are label prefixes within a switch body, not braceless control-flow constructs. Without this reset, `switch(0) case 0: int x;` would erroneously set `body_close_idx` on `x`, artificially shrinking its lifetime in the CFG verifier and allowing gotos to bypass the declaration. Without this, the declaration would inherit the enclosing function scope's lifetime, causing the CFG verifier to falsely reject forward gotos past already-dead declarations. Note: backward gotos to a label *before* a VLA declarator are legal per C11 §6.2.1p7 (variable scope begins after declarator completion), so the label is outside the VLA's scope and no checking is needed for the backward direction.

**`pparse_skip_one_stmt_impl` scratch buffers:** The statement-end scanner uses three thread-local scratch arrays: `do_if_save` / `do_tn_save` / `do_snap_start` (sized `pparse_sos_do_cap`), `do_snap_buf` (sized `pparse_sos_snap_cap`), and `if_trail_snap` (sized `pparse_sos_if_cap`). They are heap-allocated on first use and grow geometrically (2× on demand) so the scanner imposes no fixed depth limit on `if`-chain length, `do`-nesting depth, or accumulated trail snapshots: pathological inputs (e.g. 1100-deep `if` chains, 140 nested `do`s) transpile cleanly. The buffers are reused across calls and intentionally leaked at thread shutdown.

**Braceless-body typedef scope narrowing:** When a `typedef` is encountered inside a braceless control-flow body (`p1d_ctrl_pending` is true), Phase 1D temporarily narrows `pparse_td_scope_close` to the braceless body's statement boundary (computed by `pparse_skip_one_stmt_impl`) before calling `pparse_typedef_declaration`. This prevents the typedef from polluting the enclosing scope: without it, `if (1) typedef float T;` would register `T` with the parent block's `pparse_td_scope_close`, masking any keyword or type named `T` for the remainder of the block. The original `pparse_td_scope_close` is restored immediately after registration.

**Braceless-body variable shadow/VLA scope narrowing:** `p1d_probe_declaration` applies the same `PPARSE_TD_SCOPE_SAVE()` / `pparse_td_scope_close = braceless_close_idx` / `PPARSE_TD_SCOPE_RESTORE()` pattern when `p1d_ctrl_pending` is true. This narrows both `PPARSE_TDK_SHADOW` entries (via `p1_register_shadow`) and `PPARSE_TDK_VLA_VAR` entries (via `typedef_add_vla_var`) to the braceless statement boundary. Without this, a variable declared in a braceless body that shadows a typedef (e.g., `if (c) int MyTypedef;`) would poison the typedef name for the remainder of the enclosing function: `pparse_is_known_typedef` would return false, causing the CFG verifier to miss VLA declarations and the zero-init engine to skip initialization. Enum constants (`PPARSE_TDK_ENUM_CONST`) and struct tags (`PPARSE_TDK_STRUCT_TAG`) are deliberately NOT narrowed: per C11 §6.2.1p4, their scope extends to the end of the enclosing block, not the braceless statement.
...
**typeof VLA detection:** Inside `pparse_type_specifier`, VLA array dimensions inside `typeof(...)` are detected by scanning for `[` preceded by a valid type-context predecessor, followed by a runtime-variable size (`pparse_array_size_is_vla`). The predecessor check is consolidated in the `pparse_is_array_bracket_predecessor` helper, which accepts: type keywords, known typedefs, type qualifiers (`PPARSE_TT_QUALIFIER`: covers `const`, `volatile`, `restrict`, `_Atomic`), `]` (multi-dim), `*` (pointer), `}` (struct/union body), or `)` (paren-pointer grouping). For identifiers (struct/union/enum tag names), the helper walks backward through `pparse_token_pool` via `pparse_walk_back(…, PPARSE_WB_PAST_NOISE)`, skipping attribute noise (`PPARSE_TF_CLOSE` balanced groups for GNU `__attribute__((...))` and C23 `[[...]]`, `PPARSE_TT_ATTR` keywords, and `PPARSE_TK_PREP_DIR` tokens) to find the effective predecessor, returning true only if it has `PPARSE_TT_SUE`. The `]` and `)` branches use the same `pparse_walk_back(…, PPARSE_WB_PAST_NOISE)` helper to find the effective token before the matching `[`/`(`, rather than using raw `pparse_token_pool[idx - 1]`. This ensures VLA detection works through arbitrary attribute noise between struct/union/enum keyword and the type name (e.g., `typeof(struct __attribute__((packed)) Tag [n])`). Parenthesized types like `typeof((int[n]))` are detected at all paren depths. However, function-pointer parameter lists are skipped: a `(` preceded by `)` signals a function parameter list (e.g., `typeof(void(*)(int[n]))`), and `[n]` inside such lists describes parameter dimensions, not the outer type. This prevents false VLA flags on function pointers: important because `register` storage class variables cannot have their address taken, making memset-based zero-init illegal for them. The same `pparse_is_array_bracket_predecessor` check is used in the `_Atomic(...)` VLA scanning loop and in the `sizeof(...)` branch of `pparse_array_size_is_vla` (which detects VLA types inside sizeof expressions like `sizeof(int const [n])`). The `_Atomic(...)` inner type classification (`is_struct`, `is_union`, `is_typedef`) also uses `pparse_skip_noise` on the first inner token, ensuring that `_Atomic(__attribute__((packed)) struct S)` correctly detects the struct and produces memset-based zero-init. **sizeof VLA param dereference detection:** VLA function parameters (marked `PPARSE_TDF_PARAM`) decay to pointers, so `sizeof(param)` is always `sizeof(pointer)`: constant. The verifier only flags a VLA param inside `sizeof` as producing a VLA-sized type when the param is dereferenced or used in an expression that accesses VLA dimensions. The adjacency check examines both the preceding and following tokens for: `*` (explicit dereference), `[` (subscript, including the commutative form `1[param]` where `[` is the *preceding* token), `+`/`-` (pointer arithmetic: catches `*(1 + param)`, `*(param - 0)` etc.), and `*` as a following token (pointer-scaled multiplication). This ensures that `sizeof(param)` alone correctly evaluates to pointer size (constant) while `sizeof(param[i])`, `sizeof(1[param])`, `sizeof(*(1 + param))`, and all equivalent ISO C expressions that derive a VLA-sized type are correctly flagged. False positives (e.g., flagging `sizeof(&param[0])` which produces a constant pointer size) only result in stricter goto checks; false negatives would cause stack corruption.
...
**is_static_storage:** Set when the declaration has `static`, `extern`, `_Thread_local`, `thread_local`, or `__thread` (GNU extension) storage class. The prescan tracks this via `p1d_saw_static` (set when skipping `PPARSE_TT_STORAGE` tokens before the type specifier, and also checked via `type.has_static` / `type.has_extern` from `pparse_type_specifier`). The CFG verifier exempts static-storage declarations from goto/switch checks because their initialization occurs at program/thread startup, not at the declaration point.
...
**K&R function fallback:** K&R-style parameter declarations (e.g., `void f(a) int a; {`) reset `file_scope_stmt_start` at each `;`. If the initial `capture_function_return_type(file_scope_stmt_start)` fails (returns 0), Phase 1E walks backward from `{` past K&R parameter declarations (`;`-separated) to find the actual parameter list `)`, then backward from that `(` to find the real declaration start, and retries capture. The backward walk uses `pparse_p1_knr_find_close_paren`, which distinguishes the function's K&R identifier-list `)` from inner `)` tokens in parameter type declarations (e.g., function pointers `void (*cb)(int, int);`) by checking whether the enclosed tokens are exclusively identifiers and commas. Non-identifier-list `(...)` groups are skipped as balanced groups.

Taint flags (`PPARSE_TT_SPECIAL_FN`, `PPARSE_TT_NORETURN_FN`, `PPARSE_TT_ASM`) remain on the `body_open` token's tag and are read directly: they are not copied into FuncMeta fields.
...
Calls `pparse_validate_defer_statement` for every `defer` keyword encountered. This function is pure: it only calls `pparse_error_tok` on violations, no mutations.
...
**Paren-aware orelse scanning:** The orelse keyword scanner inside `pparse_validate_defer_statement` recurses into balanced `(…)` and `[…]` groups via `pparse_defer_scan_orelse_in_group` instead of blindly skipping them with `pparse_pair`. This is critical because Phase 1D's `p1d_scan_init_orelse` strips outer parentheses from initializers (macro-hygiene pattern), making `defer { int x = (0 orelse return); }` reachable at runtime. Without recursion, the `return` inside parens was invisible to Phase 1F. `{` groups are still skipped (statement expressions are handled separately by `pparse_defer_scan_hidden_stmt_exprs`).

**Chain-aware scanning:** Both `pparse_defer_scan_orelse_in_group` and the inline orelse scanner in `pparse_validate_defer_statement` scan ALL orelse instances in their range: they do not stop after finding the first orelse. This is critical because chained orelse like `(1 orelse 0 orelse return)` or bare `int x = f() orelse 0 orelse goto err;` in defer bodies hides the dangerous control-flow keyword behind a benign intermediate orelse. Each scanner uses `PParseToken *prev` tracking with `pparse_orelse_shadow_is_kw(prev)` positional disambiguation to avoid false positives on variables named `orelse`. After validating each orelse's action (via `pparse_validate_defer_control_flow` for keywords, or recursive `pparse_validate_defer_statement` for block-form `orelse { … }`), the scanner advances past the action tokens and continues scanning for additional orelse instances.

**Control-flow condition head scanning:** `pparse_skip_defer_control_head` (used to advance past the `(…)` condition of `if`/`while`/`for`/`switch`/`do-while`) calls `pparse_defer_scan_hidden_stmt_exprs` on the condition parens before skipping past them. This ensures that statement expressions inside control-flow conditions (e.g., `if ( ({ return 1; 1; }) )`) are scanned for forbidden control-flow keywords. Without this, `return`/`goto`/`break`/`continue` hidden inside a statement expression in a condition head would bypass Phase 1F validation entirely.

**Case/default label scanning:** When `pparse_validate_defer_statement` encounters a `case` or `default` label, it scans forward to the terminating `:` using ternary depth tracking (`?` increments, `:` at `td > 0` decrements, `:` at `td == 0` terminates). This correctly handles `case 1 ? 2 : 3:`, matching the pattern used by Phase 1D and `pparse_skip_one_stmt`. The scan also stops at bare `}` tokens (`PPARSE_TF_CLOSE && ch0 == '}'`), same boundary guard as the orelse and statement-expression scan loops, preventing overshoot into the outer scope on malformed case labels with missing `:`.

**Braceless defer body declaration with `orelse` initializer rejection:** Phase 1D walks braceless defer bodies (`defer STMT;`) in the `!at_stmt_start` branch of the main prescan loop, so the type keyword starting a declaration inside such a body never receives the `P1_IS_DECL` annotation. Pass 2's `try_zero_init_decl` then bails at the `P1_IS_DECL` gate, and `emit_deferred_orelse` takes over, treating the type keyword as part of the bare-orelse LHS and emitting an invalid `__typeof__(int t) __prism_oe_N = ...`. `p1d_validate_defer` rejects the case at Phase 1F: when the defer body is non-block (`!pparse_match_ch(body, '{')`) and starts with a type-specifier-bearing tag (`PPARSE_TT_TYPE | PPARSE_TT_QUALIFIER | PPARSE_TT_SUE | PPARSE_TT_TYPEOF | PPARSE_TT_BITINT | PPARSE_TT_STORAGE`), the scanner walks the body's depth-zero tokens for `pparse_is_orelse_kw_shadow`. If found, a hard error directs the user to wrap the body in braces. The block form (`defer { int t = 0 orelse c; }`) works because `handle_open_brace` enters the block normally and Phase 1D annotates the inner declaration at stmt_start.

**Braceless defer body declaration: zero-init annotation:** When the braceless body starts with a type specifier but contains **no** `orelse` in the initializer (e.g. `defer int t;`), `p1d_validate_defer` runs `pparse_type_specifier` on the leading token and sets `P1_IS_DECL` on that type-start token when zero-init is enabled and the declarator sits in real block scope (`brace_depth > 0`). That mirrors the annotation `p1d_probe_declaration` would apply at `at_stmt_start`, without moving Phase 2 semantic gates into Pass 2.

**`pparse_skip_to_semicolon` boundary hardening:** `pparse_skip_to_semicolon(tok, end)` accepts an optional `end` boundary pointer and stops at bare `}` tokens (`PPARSE_TF_CLOSE && ch0 == '}'`) in addition to `;` and `EOF`. Without the `}` stop, a missing semicolon inside `defer { int x = 5 }` caused the scanner to walk past the closing `}` into the outer scope, making `pparse_validate_defer_statement`'s `tok != end` exact pointer test silently overshoot: any `return` in the enclosing function was then falsely flagged as "return inside defer block". Three defense layers: (1) `pparse_skip_to_semicolon` stops at bare `}`, (2) `pparse_validate_defer_statement`'s block loop uses `pparse_idx(tok) < end_idx` as overshoot guard, (3) the orelse and statement-expression scan loops inside `pparse_validate_defer_statement` also check for `}` boundary escape. All callers outside defer validation pass `NULL` for `end` (no boundary).

**Forbidden function contexts:** Defer in functions that use `setjmp`/`longjmp`, `vfork`, or `asm goto` (computed labels inside assembly) is rejected via `PPARSE_TT_SPECIAL_FN`/`PPARSE_TT_NORETURN_FN`/`PPARSE_TT_ASM` tag bits on the function body's opening `{` token (checked as `func_meta[idx].body_open->tag`). The `PPARSE_TT_SPECIAL_FN` taint covers all standard and implementation-internal variants that survive preprocessing: `setjmp`, `_setjmp`, `__setjmp`, `__sigsetjmp`, `longjmp`, `_longjmp`, `__longjmp`, `__siglongjmp`, `__longjmp_chk`, `sigsetjmp`, `siglongjmp`, `__builtin_setjmp`, `__builtin_longjmp`, `__builtin_setjmp_receive`, `pthread_exit`, `savectx`. Detection is token-name-based: even bare references like `void (*fp)(jmp_buf, int) = longjmp;` taint the function because the `longjmp` token retains its `PPARSE_TT_SPECIAL_FN` tag. The only undetectable bypass is a function pointer passed from another translation unit (cross-TU indirect call), which is inherent to single-TU static analysis.

---

### 3.7 Phase 1G: Orelse Classification & Annotation

**Function:** `p1d_classify_bracket_orelse_ex` (brackets) and `p1d_annotate_typeof_orelse` (typeof), driven from `p1_full_depth_prescan`, `p1d_probe_declaration` (via `p1d_classify_decl_dims` for declarator dims), and `p1d_scan_balanced_group` (for `[`/`typeof` inside skipped `(…)` groups: cast operands, `sizeof`, parameter lists, typedef walks).

Every `orelse` that is the Prism keyword receives `P1_IS_ORELSE_KW`; bracket-dimension orelse additionally receives `P1_OE_BRACKET` (on the keyword and the enclosing `[` / typeof `(`), and declaration-initializer orelse receives `P1_OE_DECL_INIT` (scanned by `p1d_scan_init_orelse`). Keyword-vs-identifier disambiguation happens **once**, here, using the positional helpers (§6.2); after annotation, every Pass 2 consumer trusts the bits.

Classification context controls strictness: `hard_ctx` (set for real declarations and statement-level brackets) rejects file-scope / struct-body / over-paren-wrapped bracket orelse; `allow_se_hoist` (set only where a local VLA declaration can hoist a temp) permits side-effect LHS: skipped groups pass `false` and reject side effects instead.

**Control-flow rejection:** An `orelse` inside `[…]` whose action is a control-flow keyword (`return`, `break`, `continue`, `goto`) or `{` (block form) is a hard error, raised here before Pass 2 emits any output.

**Chain-aware side-effect rejection (Phase 1G + Phase 1D):** When two or more `orelse` tokens appear at depth 0 inside the same `[…]` (chained bracket orelse), the tokens between consecutive depth-0 orelses are checked via `reject_orelse_side_effects` (the intermediate LHS is emitted as `(LHS) ? (LHS) : (RHS)`: duplication must be side-effect-free). Nested orelse (inside a group within the brackets) checks the innermost group's LHS, including the callee for call arguments. `p1d_annotate_typeof_orelse` applies the same per-link scan to every chained orelse inside `typeof(…)`.

**File-scope guard:** Bracket orelse outside any function body (`p1d_cur_func < 0`) is a hard error: hoisting a temp variable requires a statement context (e.g., `void process(int buf[1 orelse 2]);` at file scope).

**Invalid context rejection (Phase 1):** Orelse inside `struct`/`union` bodies, inside `enum` bodies (enum constants must be compile-time integer constants), and inside `_BitInt(N)` / `_Alignas(N)` (compile-time constant expression contexts) is caught and errored. The `_BitInt`/`_Alignas` check fires inside `pparse_type_specifier` (called from Phase 1D prescan).

**Bare-expression orelse** (`OE_BARE_ASSIGN`, `OE_BARE_ACTION`, `OE_BARE_COMPOUND`) is validated by `p1d_validate_bare_orelse` (side effects, missing action, including per-chain-link `orelse ;` / `orelse ,` empty fallbacks: LHS indirection rules, preprocessor spans) and annotated including all depth-0 chain members; the *lowering shape* is still chosen at emit time in Pass 2, which is safe because the symbol table is immutable.
...
Runs after all Phase 1 sub-phases complete. Gated by `PPARSE_F_DEFER | PPARSE_F_ZEROINIT`: when neither feature is enabled, only functions containing VLA declarations are verified (all others are skipped). For each function's `P1FuncEntry[]` array, performs an O(N) linear sweep with label hash table and monotonic watermark arrays. The label hash table is allocated before `pparse_arena_mark` so it persists in `FuncMeta` for O(1) label lookup in Pass 2; only the watermark arrays and forward-goto list are reclaimed via `pparse_arena_restore`.
...
3. **Forward goto:** When a `P1K_GOTO` is encountered and its target label has not yet been seen, add it to a forward-goto hash table (separate-chaining, keyed by label name). When the target `P1K_LABEL` is reached, resolve by looking up the hash chain for O(1) amortized resolution, then checking all `P1K_DEFER` and `P1K_DECL` entries between the goto and label positions. If any dangerous entry's scope is an ancestor-or-self of the label's scope, error. Scope exits are pre-computed via `pparse_scope_block_exits` (O(depth) LCA tree walk) and stored in `P1FuncEntry.label.exits` for O(1) retrieval in Pass 2.
...
5. **Switch/case:** On `P1K_SWITCH`, snapshot the current watermarks. On `P1K_CASE`, compare current state against the switch's snapshot. Defers in ancestor-or-self scopes that were not active at the switch entry → error (defer skipped by fallthrough). Non-VLA declarations in nested blocks: if the declaration has a **user initializer** (`has_init`), **always** error (case labels must not bypass explicit initialization: independent of `-fno-zeroinit`). Otherwise, bypass of Prism zero-init is only checked when `PPARSE_F_ZEROINIT` is on (unless `raw` or static storage duration).

6. **Statement-expression boundary:** Gotos into a GNU statement expression are rejected. When resolving a forward or backward goto, if the target label is inside a `is_stmt_expr` scope and the goto is not within that same statement expression (checked via `pparse_scope_stmt_expr_ancestor`), a hard error is raised. Gotos *out of* a statement expression are allowed (GCC/Clang support this and Prism's defer+goto idioms rely on it).

7. **Computed goto / asm goto:** If `has_computed_goto` is set or the function body's opening `{` has `PPARSE_TT_ASM` (asm goto taint), and the function has `P1K_DEFER` entries (when `PPARSE_F_DEFER`) or VLA `P1K_DECL` entries or non-`raw`, non-static-storage, **non-user-initialized** `P1K_DECL` entries (when `PPARSE_F_ZEROINIT`), hard error: the jump target cannot be verified at compile time. `asm goto` labels are inside the assembly string and cannot be extracted by the token walker, making CFG verification impossible. Declarations with an explicit `=` initializer (`P1FuncEntry.decl.has_init == true`) are exempt from the zeroinit gate: the user has taken over initialization explicitly, and Prism does not synthesise auto-zero-init that a computed goto could bypass.
...
**Switch unbraced declaration exemptions:** Phase 1D and Pass 2 both reject the trapdoor pattern `switch (x) { int z; case 0: … }` (a plain auto declaration directly in the braced switch body, where Prism's auto-zero-init synthesised at the declaration point would be skipped by every `case` jump). The degenerate braceless form `switch (x) int z;` is accepted: no `case` label can exist, so the body is unreachable dead code and the zero-init cannot be bypassed-then-read. Both gates are restricted to the *plain* form: a declaration with **explicit user intent** is exempt and passed through verbatim. Explicit intent flags (any one is sufficient): an `=` initializer (`has_init`), `raw`, `typedef`, `struct`/`union`/`enum` reference, `static`, `extern`, `_Thread_local`, `register`, `_Atomic`, `constexpr`, or `_Alignas`. The `_Alignas` flag is tracked by a dedicated `PParseTypeSpec.has_alignas` bitfield set in `pparse_type_specifier` when a `PPARSE_TT_ALIGNAS` token is seen. Phase 1D additionally consults its `saw_static` parameter: storage-class specifiers are consumed by the Phase 1D main loop *before* `p1d_probe_declaration` runs, so the `type.has_static` flag from `pparse_type_specifier` would not see them otherwise. Actual case-skips-decl violations (a real jump past an initialized declaration in a braced switch body) are still caught by the CFG verifier's `P1K_CASE` handler, which fires regardless of the unbraced gate.

**GNU nested function gating:** GCC nested function definitions inside an outer function are passed through verbatim as plain GNU C *unless* (a) the outer function uses `defer`, or (b) the nested body itself uses `defer`/`orelse`. Case (a) conflicts with Prism's defer epilogue vs GCC trampolines; case (b) would misscompile because nested bodies are not lowered. Both are hard errors. Plain GNU C nested-function idioms without Prism keywords, including statement-expression nested definitions like `int (*fp)(int) = ({ int g(int x) { return x + 1; } g; });`: remain accepted. The forward scan for outer `defer` covers keywords appearing both before and after the nested function definition in source order.

**typeof / `pparse_scan_paren_for_vla`:** Inside `typeof(...)` only (`check_typeof == false` in the scanner), a `sizeof(...)` operand is skipped as a balanced group so `sizeof(int[n])` does not mark the enclosing `typeof` as variably-modified for multi-decl / split detection. `_Atomic(...)` passes `check_typeof == true` and continues to inspect `sizeof` interiors for VM brackets.
...
O(N) per function where N is the number of `P1FuncEntry` items, plus O(depth) for `pparse_scope_is_ancestor_or_self` checks.

**PParseArena management:** Per-function temporary allocations (watermark arrays, forward-goto list) are reclaimed after each function via `pparse_arena_mark`/`pparse_arena_restore`, preventing O(total_entries) memory accumulation across all functions in a translation unit. The label hash table is allocated **before** `pparse_arena_mark` and persists in `FuncMeta.label_hash` for O(1) label lookup in Pass 2.
...
|---|---|---|
| `handle_goto_keyword` | `PPARSE_TT_GOTO` | Emit defer cleanup (LIFO unwinding to target label depth), emit `goto`. Safety checks are in Phase 2A. |
...
| `p1_label_find` | `PPARSE_TT_GOTO` dispatch | O(1) label lookup via `FuncMeta.label_hash` (persisted from Phase 2A); falls back to linear scan if no hash. |
| Orelse handlers | `PPARSE_TT_ORELSE` | Multiple handlers for bracket, decl-init, block, bare-assign, bare-action, bare-compound forms. |
...
- `last_emitted` is `;` or `}`, and
- the next token matches `emit_token_starts_decl_like` (type/storage/attribute starts, typedef names, etc.), and
- the next token is **not** already `PPARSE_TF_AT_BOL`.

Suppressed **inside aggregate member lists**: `in_struct_body()` (scope stack marks `= { … }` initializers and keyword-headed aggregates opened via `handle_open_brace`), and `pparse_ctx->aggregate_member_nest`, incremented when emitting `{` whose **predecessor** carries `PPARSE_TT_SUE` (`struct`/`union`/`enum`), decremented on `}`, so anonymous SUE bodies inside balanced walkers (e.g. `typeof_unqual(union { int a; long b; })`) are not split.

`handle_open_brace` tags keyword-headed aggregates using the `{` token’s **pre-open** predecessor (`tok_before_brace`), not `last_emitted` after emitting `{` (that token is punctuation with `PPARSE_TT_STRUCTURAL`, not `PPARSE_TT_SUE`).

### `orelse` value fallback on volatile / `_Atomic` declarations

Declaration-initializer value fallback (`T x = expr orelse fb`) must not lower to `x = x ? x : fb` when **`volatile`** or **`_Atomic`-qualified** storage would make the double read of `x` observable. `emit_orelse_action` uses `if (!x) { x = fb; }` instead (`single_eval_lhs`). `_Atomic` in the type specifier sets `PParseTypeSpec.has_atomic`; typedefs defined with `_Atomic(...)` set `PParseTypedefEntry.is_atomic`, exported via `PPARSE_TDF_ATOMIC` / `pparse_typedef_apply_tdf_flags`, so names like `typedef _Atomic(int*) atomic_ip_t` inherit the same behavior.

**Volatile/atomic pointer + compound-literal `orelse`:** Assigning the address of a compound literal through the `if (!p) p = &(T){init};` shape would leave `p` dangling when the literal’s lifetime ends at the substatement (C11 §6.8.4.1p2). Pass 2 rejects `&( … ){ … }` fallbacks when the target uses single-eval lowering (`emit_orelse_action`); use block-form `orelse { … }` with a named object or a non-literal fallback.
...
Scope exits between a goto and its target label are pre-computed during Phase 2A via `pparse_scope_block_exits` (an O(depth) LCA tree walk on the scope tree) and stored in `P1FuncEntry.label.exits`. Pass 2 retrieves these via `p1_goto_exits` using a monotonic cursor through the goto entries (O(1) amortized).
...
**Counter capture in `emit_return_body`:** `pparse_ctx->ret_counter` is the shared monotonic ID pool that backs `__prism_ret_N`, `__prism_oe_N`, `__prism_dim_N`, `__prism_ret_t_N`, `__prism_p_N`, and `__prism_i_N`. `emit_return_body` reserves its return-temp ID up-front (`unsigned ret_id = pparse_ctx->ret_counter++`) before calling `emit_all_defers()`, then uses `ret_id` for both the `__prism_ret_<N> = (...)` declaration and the `return __prism_ret_<N>;` reference. Without the up-front capture, defer-body emission inside `emit_all_defers()` would consume additional counter slots for any orelse/dim temps it produces, desyncing the declaration from the reference and emitting `return __prism_ret_<M>` referring to an undeclared identifier.

`emit_deferred_range` handles defer bodies by delegating to `emit_statements(start, end, EMIT_DEFER_BODY)`, which provides bare orelse, raw stripping, zero-init, control-flow keyword tracking, and SUE body handling within deferred code. **Deferred-range boundary clamp:** Both `emit_bare_orelse_impl` and `emit_deferred_orelse` consume the trailing `;` of the orelse statement and advance `t = pparse_next(t)`. When called from a deferred-range emission, that `;` *is* the `end` boundary of the defer body; advancing past it makes `emit_statements`' `tok != end` loop overshoot, spilling user tokens following the defer (the next statement, the function body's closing `}`) into the emitted defer body. Both emitters clamp via `if (end && pparse_idx(t) > pparse_idx(end)) t = end;` after the trailing-`;` advance, restoring the boundary so the caller's loop terminates naturally. The colon (`:`) label handler in `emit_statements` fires in both `EMIT_NORMAL` and `EMIT_DEFER_BODY` modes: it resets `at_stmt_start = true` after case/default labels so that subsequent orelse and zero-init processing is correctly triggered. The handler uses `pending_case_colon` to support complex case label expressions like `case (2+3):`, `case (FOO):`, or `case (int){42}:` where `last_emitted` before the `:` is `)` rather than an identifier or number: without this flag, the `)` would not match the `pparse_is_identifier_like || PPARSE_TK_NUM` check and `at_stmt_start` would not be reset, causing subsequent declarations to miss zero-initialization. The flag is set when `PPARSE_TT_CASE | PPARSE_TT_DEFAULT` is encountered and consumed (or cleared) at the `:` handler. Both `emit_statements` and `transpile_tokens` maintain independent `pending_case_colon` flags. Phase 1F bans user labels (`ident:`) in defer bodies (hard error: "labels inside defer blocks produce duplicate labels"), so only case/default labels reach this handler in defer mode.
...
`check_defer_var_shadow` detects when a newly-declared variable name appears in an active defer body: this would silently capture the wrong variable at cleanup time. Uses a two-tier PParseHashMap architecture for O(1) amortized lookups:
...
2. **`FuncMeta.defer_body_captures`:** per-defer-body capture sets. Maps `pparse_idx(body)` → `PParseHashMap*` where each inner PParseHashMap contains the exact set of names captured by that specific defer body. Used for O(1) per-body lookups via `defer_body_has_capture(func_idx, body, name, nlen)`.

Both HashMaps are populated during Phase 1D by `defer_body_populate_captures(body, body_end, out, body_captures_map)`, a single-pass O(M) scanner that computes the exact set of captured (free) variables for each defer body. A captured variable is one referenced in the defer body but not locally declared within it at the reference's scope depth or deeper. The scanner maintains per-name state in a local PParseHashMap (`name → decl_depth`), a scope-exit stack for resetting local declarations when `}` closes a brace depth, and for-init scope tracking for C99 `for(TYPE name = ...)` declarations. When an inner scope redeclares an identifier, the stack records `had_binding` and `prev_val` **before** overwriting the slot; popping the inner depth restores the previous binding via `pparse_hashmap_put` or `pparse_hashmap_remove` (tombstones in `parse.c`), so outer declarations are never confused with captures after the inner block ends.

**`pparse_scan_paren_for_vla` / `typeof`:** `pparse_scan_paren_for_vla` registers enum constants defined inside `typeof(...)` / `_Atomic(...)` argument lists via `pparse_enum_constants` when an `enum { ... }` body is encountered, so downstream bounds checking and typedef shadow logic see ghost enumerator names. The `pparse_type_specifier` `PPARSE_TT_TYPEOF` qualifier scan treats inline anonymous struct bodies (`struct { ... }` after `struct`/`union`) as full struct bodies for `pparse_struct_body_contains_volatile`.

**K&R definitions:** After registering identifier-list parameters from `(...)`, `p1_register_knr_param_vlas` scans declaration statements between the closing `)` and the function `{` for VLA bracket syntax on parameters (e.g. `int a[n];`), registering them as VLA parameters for CFG / bounds tracking.

**`-fbounds-check` unary `&`:** After resolving the array token for a subscript, `try_bounds_check_subscript` peels one parenthesis layer `(array)` before applying the unary-`&` exemption for one-past-the-end indices (so `&(arr)[n]` matches `&arr[n]`).

**Volatile/atomic pointer + compound-literal `orelse`:** Same dangling-literal rejection as §5 Pass 2 (`emit_orelse_action`, single-eval LHS targets). Block-level context detection uses a per-block base paren depth array (`block_base_pd[bd]`): each `{` records the current paren depth as its block's baseline, and declarations are recognized only when `pd == block_base_pd[bd]`: the paren depth matches the block's entry depth. This correctly handles GNU statement expressions nested inside arbitrary parenthesized expressions (e.g., `(1 + ({ int x = 1; x; }))`) where the stmt-expr's block opens at `pd > 0`. For single-statement defers (`defer f();`), `body` points directly to the first statement token (no `{`), so `bd` stays 0 and the local-declaration logic correctly doesn't fire: single-statement defers have no block scope for local declarations.
...
The scan covers two ranges: enclosing-scope defers `[0, blk->defer_start_idx)` and same-block defers `[blk->defer_start_idx, defer_count)`. Enclosing-scope matches register a `DeferShadow` entry for deferred checking at control-flow exits (`check_defer_shadow_at_exit`). Same-block matches are rejected by Phase 1 (`p1_check_defer_same_block_shadow`): the shadowing variable is unconditionally live when the defer fires at block end, so no control-flow analysis is needed; `check_defer_var_shadow` keeps the same immediate error as a `PRISM_DEBUG` assert. For-init declarations are exempt from the same-block immediate error (they use deferred `DeferShadow` checking instead) because their implicit scope ends at the loop boundary. A token-index range guard (`var_idx ∈ [defer.stmt, defer.end)`) prevents false positives from declarations inside defer bodies matching against their own body (e.g., `defer { char buf[16]; ... }`).

**Statement-expression shadow isolation:** `DeferShadow` entries use `block_depth` for scope tracking. For-init shadows are registered at `block_depth + 1` (anticipating the loop body). When a statement expression inside a for-paren pushes a `SCOPE_BLOCK`, its depth equals the for-init shadow's depth. On `scope_pop`, the `>=` cleanup would prematurely clear the for-init shadow. Fix: `ScopeNode` stores `saved_defer_shadow_count` at stmt-expr open time (`handle_open_brace`). `scope_pop` restores `defer_shadow_count` to at least this saved value when closing a stmt-expr `SCOPE_BLOCK`, preventing enclosing shadows from being cleared by inner scope transitions.
...
- **Pass 2 (enclosing-scope):** Enclosing-scope enum shadows are deferred to `check_defer_var_shadow`, which queues a `DeferShadow` entry checked only at control-flow exits (`check_defer_shadow_at_exit`). This correctly allows inner-scope enum constants that shadow defer-captured names when no control-flow exit is reachable while the shadow is live. Three Pass 2 hooks cover expression-context enums: (1) `walk_balanced` emits `check_enum_body_defer_shadow` for `pparse_is_enum_kw(t)` with a body brace when `defer_count > 0`; (2) `walk_balanced_orelse` has the same hook in its no-orelse flat emit loop for array dimension enums; (3) the main loop's statement-start `check_enum_typedef_defer_shadow` covers top-level enum definitions.

**Control-flow paren keyword detection:** `p1d_scan_balanced_group` also detects `defer` and `orelse` keywords inside control-flow condition parentheses (`if(…)`, `while(…)`, `for(…)`, `switch(…)`). When the preceding saved token has `PPARSE_TT_IF | PPARSE_TT_LOOP | PPARSE_TT_SWITCH`, any `defer` or `orelse` at statement-expression depth zero (`se_depth == 0`) is rejected with a hard error. The `se_depth` tracker (not raw `inner_depth`) is used so that keywords inside GNU statement expressions within the condition (e.g., `if (({ int x = f() orelse 0; x; }))`) are correctly allowed: the stmt-expr provides its own block scope. `se_depth` is maintained via a 64-entry `se_close_stack`: on `({` entry, the matching `}` is pushed and `se_depth` increments; on reaching that `}`, the stack pops and `se_depth` decrements.

**Expression-context defer rejection:** `walk_balanced`, `walk_balanced_orelse`, and `emit_token_range_orelse` reject unshadowed `defer` keywords in their fallthrough emit paths (array dimensions, `typeof(…)` internals, balanced paren/bracket groups). The check fires when: (1) `PPARSE_TT_DEFER` tag, (2) `last_emitted` does not have `PPARSE_TT_MEMBER` (distinguishes `s.defer` member access from bare `defer`), (3) not shadowed by a typedef/variable entry (`!pparse_typedef_lookup(t)`) or block-form (`pparse_next(t) == '{'`), and (4) followed by an identifier or `{`: the only valid `defer`-as-keyword patterns. Condition (4) prevents false positives on function names in attribute arguments (e.g., `__attribute__((cleanup(defer)))`) where `defer` is followed by `)`. This complements the `check_orelse_in_parens` scanner (which catches `defer` at the top level of `(…)` groups) by covering `[…]` array brackets and nested balanced groups where `defer` would otherwise leak raw to the C backend.
...
**Limitation:** Does not support struct/union values (no whole-struct zero check). Struct/union pointers work. Rejected in Phase 1 for both declaration form (`T x = e orelse …`) and bare assignment (`x = e orelse …`, including `*p = get() orelse …` when `get` returns a struct/union by value). Typedeffed structs in declaration form are rejected when `pparse_type_specifier` sets `is_struct` (`PPARSE_TT_SUE` or `PPARSE_TDF_AGGREGATE`). `typeof(struct S *)` / `_Atomic(struct S *)` are treated as pointers (outer `*` clears the struct-value mark). Opaque `typeof(expression)`: where the expression is a variable name (e.g. `typeof(my_var)`) or a function call (e.g. `typeof(make())`) whose type is an aggregate: cannot always be detected at the token level. The backend compiler reports a clear error (`used type ... where arithmetic type is required` or `wrong type argument to unary '!'`).
...
| Expression subscript | `arr[i orelse 0] = 1;` | Index rewritten in place to `(i) ? (i) : (0)`: works in return statements with active defers too |
...
**Invalid contexts:** Rejected in Phase 1, with `PRISM_DEBUG` assert shells in Pass 2:
...
**Keyword shadow disambiguation (annotation model):** When the identifier `orelse` is shadowed by a variable, enum constant, or typedef name (`PPARSE_TT_ORELSE` tag co-exists with a typedef-table entry), positional context decides keyword vs identifier. The decision is made **once, in Phase 1**, and baked into the `P1_IS_ORELSE_KW` annotation bit; Pass 2 trusts the bit and never re-derives the answer.

- `pparse_orelse_shadow_is_kw(prev)`: the core positional predicate. Given the token *before* the ambiguous `orelse`, returns true when context indicates keyword usage: preceded by an expression-ending token (`)`, `]`, identifiers, numbers, `}`, postfix `++`/`--`). Returns false for type keywords, qualifiers, storage classes, struct/union/enum, `typeof`/`_BitInt`/`_Alignas`, `inline`, attributes, and all non-expression-ending tokens. The `PPARSE_TT_MEMBER` check (`.`/`->` before `orelse`) is performed by the callers. Wrapped by `orelse_kw_at_bare` / `orelse_kw_at_shadow` (which add the typedef-table lookup, known-function-call, and label/goto-target guards).
- These positional helpers run in the Phase 1 classification sites (§3.7): bracket/typeof classification, decl-init scanning, bare-statement validation: each of which sets `P1_IS_ORELSE_KW` on confirmed keywords. Live positional checks after annotation exist only where Phase 1 legitimately could not annotate (e.g. `emit_bare_orelse_impl`'s fallback-chain rescan).
- `is_orelse_keyword(tok)`: Pass 2's unified test: a single `P1_IS_ORELSE_KW` bit read. Valid only after Phase 1 annotation has run.

**Annotation consumers:** Post-annotation scanners identify orelse keywords by testing `P1_IS_ORELSE_KW` (plus `P1_OE_BRACKET` / `P1_OE_DECL_INIT` where the form matters) instead of re-running positional checks: `find_bare_orelse`, `orelse_has_chain`, `scan_bracket_orelse`, `scan_decl_orelse`, `emit_token_range_orelse`, `try_bracket_orelse` / `try_typeof_orelse` (which test `P1_OE_BRACKET` on the opening `[` / typeof `(` directly: no content rescan), and `emit_decl_init_walk`. `find_bare_orelse` and `orelse_has_chain` retain a live positional path gated on `!pparse_p1_typedef_annotated`, because Phase 1 itself calls them during discovery, before annotation bits exist. `check_orelse_in_parens` still scans positionally (it validates contexts where orelse is *forbidden*, so no annotation exists); all its call sites use `(pparse_feat(PPARSE_F_ORELSE) || pparse_feat(PPARSE_F_DEFER))` so defer-in-paren validation fires even when `-fno-orelse` is set.
...
**Declaration prefix handling:** `try_zero_init_decl` skips storage class specifiers (`static`, `extern`, etc.), `inline`, and `__extension__` to reach the actual type keyword. The P1_IS_DECL fast gate probes past these prefixes (`PPARSE_TT_STORAGE | PPARSE_TT_INLINE | PPARSE_TT_SKIP_DECL`) to find the Phase 1D annotation on the type-start token, ensuring `__extension__ int x;` and `__extension__ struct S s;` receive zero-initialization correctly. Phase 1D's prescan also skips `__extension__` (tagged `PPARSE_TT_INLINE`) at statement start before type parsing. **For-init prefix handling:** `p1_scan_init_shadows` places the `P1_IS_DECL` annotation on the type-start token, not the first token of the declaration. When `__extension__` or storage-class specifiers precede the type keyword in a for-init declaration (e.g., `for (__extension__ int i = 0; ...)`), the annotation placement loop skips `PPARSE_TT_STORAGE | PPARSE_TT_INLINE` tokens before annotating. Without this skip, `P1_IS_DECL` would land on the `__extension__` token, and Pass 2's fast gate (which probes past `PPARSE_TT_STORAGE | PPARSE_TT_INLINE | PPARSE_TT_SKIP_DECL` to find the annotation) would never see it: silently dropping zero-initialization for the for-init variable. Similarly, `p1_scan_init_shadows` skips `PPARSE_TT_INLINE`-tagged tokens before checking the `PPARSE_TT_TYPEDEF` tag dispatch, so that `for (__extension__ typedef int T; ...)` correctly routes to `pparse_typedef_declaration` instead of falling through to the variable declaration path.
...
**Union memset:** C11 §6.7.9p21 ("all elements that are not initialized explicitly … are initialized implicitly the same as objects that have static storage duration") applies only to aggregates (C11 §6.2.5p21: arrays and structures). Unions are NOT aggregates per ISO, so the standard does not mandate that `union U u = {0}` zero the bytes beyond the first member. **In practice**, gcc-15 and clang-19 fully zero the entire union storage (verified empirically at `-O0` and `-O2` against deliberately polluted stacks for unions up to 64 bytes); Prism therefore relies on `= {0}` only in C23 init-statement contexts where a delayed `__builtin_memset` cannot be inserted (see "Init-statement zero-init via = {0}" above). For all other union declarations Prism continues to emit `__builtin_memset(&u, 0, sizeof(u))` so the result is standard-portable rather than implementation-defined. Detection: `PParseTypeSpec.is_union` is set when `pparse_type_specifier` encounters a `PPARSE_TT_SUE` token with `ch0 == 'u'`, and propagated through the typedef table via `PPARSE_TDF_UNION`. In `process_declarators`, `is_union_type = type->is_union && !decl.is_pointer` feeds into `needs_memset`. Phase 1D mirrors this via `p1d_is_union_type` in `any_would_memset`.
...
**register union rejection:** `register union U u;` is rejected with a hard error: `register` forbids address-taking (ISO C11 §6.5.3.2p1) making `memset` impossible, and `= {0}` only initializes the first named member (C11 §6.7.9p17), leaving remaining bytes indeterminate. The user must remove `register` or use `raw` to opt out. Phase 1D mirrors this rejection. `_Atomic(union U)` propagates `is_union` through the `_Atomic(type)` handler in `pparse_type_specifier`: both direct `_Atomic(union U)` and `_Atomic(UTypedef)` where `UTypedef` is a union typedef carry the `is_union` flag (queried via `PPARSE_TDF_UNION`).

**const unavoidable-memset rejection:** `const`-qualified declarations requiring memset where memset is *unavoidable* are rejected with a hard error: modifying a `const`-defined object is undefined behavior (ISO C11 §6.7.3p6). "Unavoidable" means the object cannot be safely brace-initialized at the declarator: (a) **union** (`= {0}` only zeros the first named member, leaving remaining bytes indeterminate per C11 §6.7.9p17), (b) **VLA** (initializer-list syntax is illegal on VLAs), or (c) **`_Atomic` aggregate**, including `_Atomic(struct/union/array)` *and* `_Atomic(typeof(T))` where `T` resolves to an aggregate (typeof preserves the type, and atomic stores must be sequenced rather than compound-init). Pass 2's gate (`reject_const_unavoidable_memset`) is `(is_union_type || effective_vla || brace_unsafe || (type->has_atomic && (is_aggregate || (type->has_typeof && (type->is_array || type->is_struct || type->is_union)))))`. Bare `has_typeof` must not stand in for aggregate: `const _Atomic typeof(int)` is a scalar and is allowed (memset, matching non-VLA `const typeof`). Phase 1D calls the same helper to satisfy the Two-Pass Invariant: every semantic error caught in Pass 2 must also be caught in Pass 1. The check evaluates `explicit_const = (type->has_const && !decl.is_func_ptr && !decl.is_pointer) || decl.is_const`, then scans const-qualified typedefs in the type specifier range when `explicit_const` is false and the declarator is not a pointer. The `!decl.is_pointer` guard is critical for ISO C type composition: in `const char *arr[n]`, the `const` qualifies the pointee (`char`), not the pointer elements, so `memset`ting the pointer array is legal. Without this guard, Prism fatally rejects standard-compliant code. Conversely, `int *const arr[n]` (const pointer, not const pointee) correctly has `decl.is_const = true`, triggering the rejection. The same `!decl.is_pointer` guard applies in `pparse_has_effective_const_qual` (used by the orelse const-fallback path): `const int *p = get() orelse 0` routes through the normal mutable-pointer ternary path, not the const-orelse temp-variable path. Non-VLA `const typeof` (e.g., `typeof(const int) a;`) is allowed because compilers optimize the memset to a compile-time initializer. **Typedef const propagation:** `pparse_typedef_declaration` records `is_const` on `PParseTypedefEntry` for base-const non-pointer types (e.g., `typedef const int CI`). Array typedefs preserve this: `typedef const int Arr[n]` sets `is_const = true` on the entry, so `Arr buf;` is correctly rejected when `buf` requires memset. Previously, `is_const` was deliberately stripped from array typedefs (`&& !decl.is_array`), allowing `memset` on `const`-qualified VLA array typedefs: undefined behavior. Chained typedefs also propagate: `typedef const int CI; typedef CI Arr[n];`: the second typedef's type specifier contains `CI` which carries `PPARSE_TDF_CONST`, propagating through the const-typedef scan. **Volatile batching performance note:** The `typeof_vars` batching mechanism passes `type->has_volatile` from the base type specifier to `emit_typeof_memsets`, which forces a byte-by-byte initialization loop (since `memset` drops `volatile`). **Typedef volatile propagation:** `pparse_typedef_declaration` records `is_volatile` on `PParseTypedefEntry` (query flag `PPARSE_TDF_VOLATILE`, macro `pparse_is_volatile_typedef`) for base-volatile non-pointer types, mirroring the const mechanism. In `process_declarators`, the type specifier range is scanned for volatile typedefs; when found (and the declarator is not a pointer or function pointer), `type->has_volatile` is set, routing the VLA through the byte loop in `emit_typeof_memsets`. Without this, `typedef volatile int VArr[n]; VArr buf;` uses `memset`, which drops the `volatile` qualifier: the compiler may then elide or reorder the initialization, violating the programmer's intent for memory-mapped or concurrent access patterns. For pointer-to-volatile declarations like `volatile int *arr[n]`, the `volatile` qualifies the pointee, not the pointer array elements: `memset` would be safe. However, because `typeof_vars` stores only variable names (not per-declarator type metadata), there is no per-variable volatile discrimination. A mixed declaration `volatile int a[n], *b[n];` forces the byte loop on both. This is a performance-only issue (correctness is preserved: the byte loop is a strict superset of `memset` behavior). **Volatile member propagation:** Structs/unions with volatile-qualified fields (direct or nested) require the byte loop even when the type specifier itself has no `volatile` keyword. `PParseTypeSpec.has_volatile_member` (separate from `has_volatile`) signals this condition. Detection: (1) `pparse_struct_body_contains_volatile()` scans struct/union bodies for `PPARSE_TT_VOLATILE` tokens and identifiers carrying `PPARSE_TDF_VOLATILE` or `PPARSE_TDF_HAS_VOL_MEMBER`; (2) `PPARSE_TDK_STRUCT_TAG` entries in the typedef table carry `has_volatile_member`, queried via `pparse_tag_lookup()` for tag-only references (`struct Foo buf[n]`); (3) typedef entries propagate `has_volatile_member` through `PPARSE_TDF_HAS_VOL_MEMBER`. In `process_declarators`, `type->has_volatile_member` (from `pparse_type_specifier`) and `pparse_has_volatile_member_typedef()` scans are merged into `type->has_volatile` for the memset vs byte-loop decision. **Struct body scanner paren-awareness:** `pparse_struct_body_contains_vla()` and `pparse_struct_body_contains_volatile()` track a `prev` token. When scanning encounters `(` preceded by a `PPARSE_TT_TYPEOF`-tagged token or an `_Atomic` token (`(PPARSE_TT_QUALIFIER | PPARSE_TT_TYPE)` both set), the scanner steps *inside* the parens instead of skipping them. This ensures `typeof(int[n])` fields are detected as VLA and `typeof(volatile int)` fields are detected as volatile. Without this, type operators inside struct bodies would be invisible to the property scanners, causing `= {0}` on VLA structs (compile error) or `memset` on volatile structs (UB per ISO C11 §6.7.3p6).

**register VLA rejection:** `register` VLA declarations without an explicit initializer (and without `raw`) are rejected with a hard error: `register` forbids address-taking (ISO C11 §6.5.3.2p1), making `memset` impossible, and VLAs cannot use `= {0}` initializer syntax. The user must remove `register` or use `raw` to opt out.

**Function-type exclusion:** `typeof(func_name)` where `func_name` is a function (not a function pointer) produces a function type: emitting `memset` on it writes to the `.text` segment (SIGSEGV). `process_declarators` detects function types via three mechanisms: (1) `pparse_is_func_typedef` scan of the type specifier for typedef'd function types; (2) `func_meta` scan matching the identifier inside `typeof(…)` against defined functions; (3) forward-declaration scan of `pparse_token_pool` at brace depth 0, detecting `ident(` patterns for functions only visible via forward declaration (not in `func_meta`). All three require a single bare identifier inside the `typeof(…)` parens. When detected, `is_func_type` is set and `needs_memset` evaluates to false. The Phase 1D prescan records function prototypes in `p1_func_proto_map` at both file scope (all `ident(` patterns) and block scope (only when the preceding token has type/qualifier/storage/SUE/typeof tag or is a known typedef). Block-scope detection uses `pparse_walk_back(…, PPARSE_WB_PAST_NOISE)` to skip GNU/C23 attribute noise between the type specifier and the function name (e.g., `int __attribute__((pure)) compute(int);`): without this, attributes cause the raw `pparse_token_pool[idx - 1]` to be `)` from the attribute, not the type keyword. **(4) Function type signatures:** `typeof(int(int))`, `typeof(void(void))`, etc.: when the inner tokens start with a type keyword and contain a `(` whose first inner token is not `*`, this is a function parameter list, indicating a function type. If the first `(` is followed by `*`, it is a pointer grouping (`typeof(void(*)(int))`), indicating a function pointer type: `is_func_type` returns false and zero-init proceeds normally. The `*` check uses `pparse_skip_noise(pparse_next(fs))` to skip GNU `__attribute__((...))` and C23 `[[...]]` attributes between the opening `(` and `*`: without this, `typeof(int (__attribute__((cdecl)) *)(int))` would be misclassified as a function type instead of a function pointer, suppressing memset. **Nested typeof skip:** The function type signature scan skips `typeof`/`typeof_unqual`/`__typeof_unqual__` keywords and their argument parentheses: `typeof(typeof(int[n]))` contains a nested `typeof(int[n])` whose `(` is a typeof argument paren, not a function parameter list. Without this skip, the inner `(` would be misidentified as a function parameter list, incorrectly classifying the VLA-preserving typeof as a function type and suppressing the required `memset`. **Array-dimension bracket skip:** The same scan also skips balanced `[...]` groups via `pparse_pair`. A `(` inside an array-dimension expression, e.g., `typeof(int[f()])` (function call as VLA size) or `typeof(int[(n)])` (paren-wrapped size, also `typeof(int[(int)n])`), is part of the dim expression, not a function parameter list. Without this skip, any `(` inside the brackets is treated as a function signature, `is_func_type` returns true, and `needs_memset` becomes false, leaking an uninitialized VLA on the stack (info leak, since `x[0]` reads indeterminate memory).
...
`goto *<expr>` is supported. `FuncMeta.has_computed_goto` is set during Phase 1D when `goto *` is detected. `asm goto` is detected by the tokenizer which sets `PPARSE_TT_ASM` on the function body's opening `{` token. During Phase 2A (`p1_verify_cfg`), if a function contains either a computed goto or asm goto alongside (a) any `P1K_DEFER` entries (when `PPARSE_F_DEFER` is enabled) or (b) any VLA `P1K_DECL` entries or (c) any non-`raw`, non-static-storage, non-user-initialized `P1K_DECL` entries (when `PPARSE_F_ZEROINIT` is enabled), a hard error is raised: the jump target cannot be verified at compile time, so defer cleanup or zero-initialization could be bypassed. **Declarations with an explicit user `=` initializer are exempt** from the zeroinit gate (`P1FuncEntry.decl.has_init`): the user has taken over initialization explicitly, so common idioms like `void *targets[] = { &&L1, &&L2 }; goto *targets[i];` (label-address arrays for computed goto dispatch) work without modification.

### 6.6 _Generic

`_Generic` expressions pass through as standard C without transformation, with one exception: when `_Generic` appears in a **declaration** context (preceded by `*`, `)`, a type/qualifier/storage/attribute/typeof/bitint token, or a known typedef) and every association names the same function with the same declarator-shaped argument list, Prism folds it to a parenthesized declarator, e.g. `extern void *_Generic(..., default: bsearch(params))` → `extern void *(bsearch)(params)`. This recovers from GCC 15+ / glibc C23 type-generic macro expansions (ISO C N3322) that otherwise leave `_Generic` in declarator-name position (file-scope and local `extern` redeclarations). Associations with distinct targets are never rewritten, so expression `_Generic` type-dispatch is preserved.

Prism tracks `_Generic` scope depth via `SCOPE_GENERIC` to prevent internal keyword processing (defer, orelse, case/default, label detection, zero-init) from firing inside `_Generic` association lists. The `in_generic()` function walks the scope stack up to the first `SCOPE_BLOCK` to detect when emission is inside a `_Generic(...)` context. The `_Generic` keyword handler and `track_generic_token` both run in **both** `EMIT_NORMAL` and `EMIT_DEFER_BODY` modes: `_Generic` inside defer bodies must push `SCOPE_GENERIC` to prevent association colons from being mis-identified as labels (which would set `at_stmt_start = true` and corrupt zero-init or orelse processing).

### 6.7 Statement expressions

GNU statement expressions `({…})` are supported. They get their own scope in the scope tree (`is_stmt_expr = true`). Declarations and zero-initialization work correctly inside them. `walk_balanced` detects `({` patterns (including when called directly on a stmt-expr `(`) and processes inner blocks with full keyword dispatch (defer, goto, return/break/continue), `try_zero_init_decl`, raw stripping, and orelse transformation (via `emit_deferred_orelse` at stmt-start with a catch-all `pparse_error_tok` for unprocessed orelse tokens, preventing literal "orelse" from leaking to the C output). The `emit_block_body` function (called from `walk_balanced` for stmt-expr processing) uses `uint32_t` for its `itag` variable (matching `PParseToken.tag`'s width) to avoid truncation of high-bit tags like `PPARSE_TT_DEFER` (1<<20), `PPARSE_TT_GOTO` (1<<17), and `PPARSE_TT_ORELSE` (1<<30). `walk_balanced_orelse` (used for array dimensions when `PPARSE_F_ORELSE` is enabled) also detects `({` patterns in its no-orelse emit path and routes them through `walk_balanced` for full transpilation. Additionally, `walk_balanced_orelse` checks for enum definitions with bodies (`pparse_is_enum_kw(t)` + `pparse_find_struct_body_brace`) in its flat emit loop when `defer_count > 0`, calling `check_enum_body_defer_shadow` to queue enclosing-scope shadow entries: this covers array dimension patterns like `int arr[sizeof(enum { handle = 1 })]`. `emit_orelse_condition_wrap` (which emits the LHS of an orelse in certain `({...}) orelse` forms) routes through `emit_range_no_prep` instead of flat `emit_tok` loops, ensuring statement expressions on the LHS receive full `walk_balanced` processing for defer/orelse/zeroinit. `walk_balanced` saves and restores `ctrl_state` on stmt-expr entry/exit to prevent stmt-expr content from corrupting the outer braceless control-flow tracking.

**Defer constraint:** A block containing `defer` must not be the last statement of the statement expression (the defer emission would overwrite the expression's return value). Prism detects this during Phase 1D's prescan walk via `pparse_p1_check_defer_stmt_expr_chain`, which walks the scope tree parent chain from the defer's scope, checking if only trivial tokens (`;`, labels, preprocessor directives, attributes with balanced parens) separate each scope close. If the chain reaches a `is_stmt_expr` scope, a hard error is raised before Pass 2 begins. Pass 2's `handle_close_brace` retains the same check as defense-in-depth (fires at `}` close time when the parent scope is `is_stmt_expr` and the next non-noise token after `}` is `}`).  The user must place the defer block before the final expression: `int fd = ({ int r; { defer cleanup(); r = work(); } r; });` or restructure without a statement expression.
...
**Detection:** A function is noreturn if declared with `_Noreturn`, `noreturn`, `[[noreturn]]`, `__attribute__((noreturn))`, `__declspec(noreturn)`, or is a known builtin (`exit`, `abort`, `_Exit`, `_exit`, `quick_exit`, `__builtin_trap`, `__builtin_unreachable`, `thrd_exit`). All occurrences of the function name are tagged `PPARSE_TT_NORETURN_FN` during tokenization.

**Injection rules:**
1. The noreturn identifier must be followed by `(` (a call, not a pointer reference)
2. After the matching `)`, only closing `)` (casts / grouping) or a trailing comma-expression may intervene before the statement `;`. So `(void)(0, die());`, `die(), 1;`, and `foo(die());` inject; `sizeof(die())`, short-circuit `x && die();` / `x || die();`, and control-paren sites (`if (die())`) do not. Unevaluated operands are excluded via `pparse_token_is_in_unevaluated_operand`
...
6. The predecessor token must NOT be a type keyword, qualifier, storage class, `inline`, `struct`/`union`/`enum`, `*`, or member operator: these indicate a forward declaration (`void abort(void);`) or struct field, not a call. `pparse_try_detect_noreturn_call` uses `pparse_walk_back(…, PPARSE_WB_PAST_NOISE)` for this backward check, skipping GNU `__attribute__((...))`, C23 `[[...]]`, and preprocessor directives. Without noise-skipping, `void __attribute__((cold)) exit(int);` has `)` as the raw predecessor, bypassing the type-keyword guard and causing `__builtin_unreachable()` to be injected after a forward declaration (not a call)
7. The identifier must NOT be shadowed by a local variable or parameter. `pparse_try_detect_noreturn_call` queries `pparse_typedef_lookup(tok)`: if a `PPARSE_TDK_SHADOW` entry exists, the call is to a local variable (e.g., a function pointer parameter named `exit`), not the global noreturn function, so injection is suppressed. Shadow registration for `PPARSE_TT_NORETURN_FN` and `PPARSE_TT_SPECIAL_FN` identifiers is handled by Phase 1C's three shadow registration sites (parameters, for-init declarations, general declarations)
...
17. Independent of `-fzeroinit`: auto-static runs through the `process_declarators` pipeline; the gate in `try_zero_init_decl` allows entry when `pparse_feat(PPARSE_F_AUTO_STATIC)` even if `pparse_feat(PPARSE_F_ZEROINIT)` is off

#### Initializer scan

`is_const_literal_init(PParseToken *eq)` performs a single forward pass over the brace-enclosed tokens. It is conservative: it rejects any token that *could* produce a non-constant value, including:

- Function calls (`ident(`)
- Variable references (any `PPARSE_TK_IDENT` not preceded by `.` and not a registered enum constant)
...
`OUT_LIT("static")` is emitted immediately before `emit_type_range` in `process_declarators` Step 3 (first declarator type emission). The existing `const` token carries `PPARSE_TF_HAS_SPACE`, providing the space between `static` and `const`.
...
1. `pparse_feat(PPARSE_F_BOUNDS_CHECK)` is on
2. After peeling supported parenthesized/postfix forms, `X` resolves to an identifier-like value token
3. `pparse_array_binding_info(X)` resolves `X` as a tracked array by merging the dedicated bounds registry with ordinary typedef/VLA bindings. Parameters, enum constants, and scoped `blocks_outer` entries for non-array or incomplete declarations take precedence over an older outer array
4. `X` is not a typedef name (`!pparse_is_known_typedef(X)`)
5. The `[` token does not carry `P1_DECL_BRACKET` (declarator brackets, tagged in Phase 1 by `decl_array_dims`, are never wrapped)
6. The `[` token does not carry `P1_UNEVAL_BRACKET`, tagged by `pparse_mark_unevaluated_brackets` for every `[` inside a parenthesized `sizeof` / `_Alignof` / `alignof` / `typeof` / `typeof_unqual` / `offsetof` / `__builtin_offsetof` operand, the controlling expression of `_Generic`, the predicate of `_Static_assert` / `static_assert`, **and every `[` inside a static-storage initializer** (file-scope, and block-scope `static` / `extern` / `_Thread_local` / `constexpr`). The first four are unevaluated operands; the last case is required because C99 §6.7.8p4 / C11 §6.7.9p4 / C23 §6.7.11p4 mandate that initializers for objects of static storage duration be constant expressions, and wrapping a subscript in `__prism_bchk(...)` turns it into a non-constant function call ("initializer element is not constant"). In `offsetof` a subscript names a struct field (unrelated to any same-named local), so wrapping would be a silent false negative or a spurious trap on VLAs.
7. The `[` is not a C23 attribute opener (`!PPARSE_TF_C23_ATTR`)
8. The token immediately before `X` is not a `.` or `->` member operator (`!(prev_tag & PPARSE_TT_MEMBER)`): `s.arr[i]` and `p->arr[i]` name a struct field, whose size is unrelated to any same-named local
...
| `m[i][j]` (2D local) | Both dims | Emit peels successive `[` and wraps each checkable rank (`dim_depth`) |
...
| `s.arr[i]`, `p->arr[i]` | No | Member subscript filtered via `PPARSE_TT_MEMBER` on prev token |
...
- **Flag:** `PPARSE_F_BOUNDS_CHECK = 256` (parse.c) and `PrismFeatures.bounds_check` (prism.c)
- **CLI:** `-fbounds-check` / `-fno-bounds-check`
- **Declarator tagging:** `pparse_declarator` identifies array dimensions; Phase 1 records `P1_DECL_BRACKET = 1 << 9` on every declarator `[`
- **Unevaluated-operand tagging:** `pparse_mark_unevaluated_brackets` sets `P1_UNEVAL_BRACKET = 1 << 10` on every `[` inside a parenthesized `sizeof` / `_Alignof` / `alignof` / `typeof` / `__typeof` / `offsetof` / `__builtin_offsetof` operand, the `_Generic` controlling expression, the `_Static_assert` / `static_assert` predicate, and any static-storage initializer (file-scope depth-0 `=`, plus block-scope declarations carrying `static` / `extern` / `_Thread_local` / `constexpr`). Runs once per TU, gated on `PPARSE_F_BOUNDS_CHECK`. File-scope initializers are detected by tracking curly-brace depth: every `=` at depth 0 opens an initializer that stays open until the matching `;`, tagging every `[` in between regardless of nesting depth. Block-scope static-storage uses a pending flag from the storage/`constexpr` keyword cleared at `;` or when entering a non-initializer `{` body. Recognition for the other cases uses `PPARSE_TF_SIZEOF` (sizeof/alignof/_Alignof/offsetof), `PPARSE_TT_TYPEOF` (typeof family), `PPARSE_TT_GENERIC`, lexeme match on `_Static_assert` / `static_assert` / `__builtin_offsetof`.
...
`noreturn void pparse_error_tok(PParseToken *tok, const char *fmt, ...)`: prints an error message with file/line/column context and calls `exit(1)`.

### pparse_warn_tok

`void pparse_warn_tok(PParseToken *tok, const char *fmt, ...)`: prints a warning. Suppressed in library mode.
...
**Response files (`@file`).** Before other CLI parsing, Prism expands GCC-style `@file` arguments (including nested `@file` up to depth 32). Tokens inside the file are whitespace-separated; double-quoted strings and backslash escapes are supported. Expansion happens *before* source detection, so a response file that lists `.c`/`.i` paths is transpiled correctly: without this, those sources would be forwarded raw to the backend and Prism extensions (`orelse`/`defer`) would misscompile. Expanded tokens are heap-owned by `Cli.rsp_owned` and freed in `cli_free`.
...
**Force-includes (`-include` / `-imacros` / MSVC `/FI` / `-Wp,-include,…` / `-Xpreprocessor -include`).** These flags are applied during Prism's preprocess step (`cc -E`) so the injected text is tokenized and transpiled. When Prism then compiles the already-expanded transpiled output, it **strips** them from the backend argv: re-passing them would inject the raw file a second time (redefinitions and untranspiled `orelse`/`defer`) or confuse the driver (`cc1: too many filenames`). Dependency-generation flags (`-MD`/`-MMD`/…) are similarly preprocess-only (see `dep_args`).

**`-M` / `-MM`.** Treated like `-E` (passthrough): Prism does not transpile via the stdin pipe, so dependency output keeps real basenames (`file.o:`) instead of `-.o:`.

**`-S`.** Sets assemble-only mode and synthesizes `file.s` when `-o` is omitted (same pattern as `-c` → `file.o`). Multi-TU `prism -c a.c b.c` / `-S a.c b.c` compiles each unit to its basename object/asm, not temp names.

**`-MD` / `-MMD` + `-o`.** Dependency flags run on the preprocess `cc -E` line (which strips `-o`). Prism injects `-MT <output>` when the user gave `-o` and did not already pass `-MT`/`-MQ`, so `.d` targets match the real binary/object name.

**`CC` / `--prism-cc=` paths with spaces.** Unquoted executable paths that exist on disk are kept as a single argv entry (and used whole for clang/gcc detection); quoted forms still pparse_tokenize via the existing splitter.

**Pipe `-fpreprocessed` / `-x none` gating.** For a single `.c` pipe compile, Prism enables `-fpreprocessed` under flatten mode, then adds `-x none` (and `-fno-preprocessed`) only when `cc_args` contains additional real input files (`.cpp`, `.s`, `.o`, …). Operands of flags that take a path argument (`-I`, `-include`, `-isystem`, `-MF`, …) are **not** treated as inputs; otherwise split forms like `-I /usr/include` or `-include hdr.h` falsely cleared `-fpreprocessed`. A user `-x none` (GCC: restore extension-based guessing) is mapped to `-x c` for the stdin pipe. When `-save-temps` / `-save-temps=*` is present, Prism skips the stdin pipe and compiles via on-disk temps instead; otherwise the backend invents invalid sidecar names like `-.i`.

**Stdin source (`-`).** A lone `-` argument is a source TU read from stdin (GCC/Clang). It is recorded in `cli.sources`, not forwarded as a raw backend input. Preprocessing stdin always supplies `-x c` (injecting it when absent, or rewriting a user `-x none`), because Clang rejects `cc -E -` without an explicit language.

**Preprocessed inputs (`.i`).** Files ending in `.i` are already preprocessed. Prism reads them directly and does **not** run `cc -E`: GCC's preprocessor emits an empty stream for `.i`, which previously dropped the whole TU (including `orelse`/`defer`). Re-feeding an already-transpiled `.i` (e.g. from `-save-temps`) skips re-injecting the bounds-check helper if `__prism_bchk` is already present.

**Non-flatten system includes.** With `-fno-flatten-headers`, Prism re-emits `#include` only for **direct** system includes entered from user code. Nested headers (e.g. glibc `bits/libc-header-start.h`) are skipped, including them alone triggers `#error`. Compiler-injected `stdc-predef.h` is also omitted.
...
**Interaction with transpile mode.** `collect_link_pragmas` runs only in `compile_sources` (compile and run modes). In `transpile` mode the directive is emitted verbatim to stdout as a `PPARSE_TK_PREP_DIR` token: consumers of the transpiled output are expected to strip or re-honour it themselves.
...
**Implementation note (post-1.1.4 fix).** `main()` aliases `Cli.cc_args` into `pparse_ctx->extra_compiler_flags` *before* `compile_sources` invokes `collect_link_pragmas`, but the pragma scanner may grow `cc_args` past its initial 16-slot cap and `realloc` the underlying buffer to a new address. Pre-fix this left `pparse_ctx->extra_compiler_flags` dangling, and the next `preprocess_with_cc` → `build_pp_argv` walk segfaulted (typically observed with ≥ ~7 macOS pragma tokens in a single source, or ≥ ~17 `*`/Linux/Windows tokens, where the first geometric realloc relocates). `compile_sources` now refreshes the alias immediately after `collect_link_pragmas` returns. The `-fno-link-pragma` flag bypasses the scanner entirely and avoids the regrowth path.
...
| Field | Role |
|-------|------|
| `start`, `end` | Offsets into the **same** `PParseFile::contents` buffer the tokenizer uses (half-open). |
...
**Minimal path inside Prism (no new deps).** Implement **(3)** first: treat sentinels as **source** of taint intervals; later, swap the producer for **(1)** or **(2)** without changing Phase 1’s consumer API (`token_macro_taint(tok)` or a small `uint32_t` on `PParseToken`).
...
Error recovery uses `setjmp`/`longjmp`: `pparse_error_tok` longjmps out, arena is reset, context pointers are NULLed.
...
- Mutate the typedef table (`pparse_typedef_add_entry` is never called)
- Unwind the typedef scope (`typedef_pop_scope` does not exist)
- Register shadows (`pparse_register_shadow` and `p1_register_param_shadows` are Phase 1 only)
- Walk the token stream for CFG safety checks (no `goto_skips_check`, no `backward_goto_skips_defer`, no `backward_goto_skips_decl`)
- Validate defer bodies (`pparse_validate_defer_statement` runs only in Phase 1F)
- Track return type state machines (`FuncMeta` provides return type data at function body entry)
- Register ghost enums during emit (`emit_tok` does not call `pparse_enum_constants`)
...
1. **Immutable symbol table:** After Phase 1B completes, the typedef table is frozen. Pass 2 performs zero mutations to the typedef table, scope tree, token annotations (`ann`), the token chain (`next_idx`: wrap-paren stripping happens in Phase 1D), or `func_meta`.
2. **All errors before emission:** Every user-triggerable `pparse_error_tok` call from semantic analysis fires during Pass 1 or Phase 2A, before Pass 2 emits its first byte, with two documented exceptions. (a) `check_defer_shadow_at_exit` requires runtime scope-stack state that only exists during Pass 2 control-flow exit emission. (b) `defer_add`'s `block_depth <= 0` guard ("defer outside of any scope") is compiled **unconditionally** (not only under `PRISM_DEBUG`): Phase 1's `p1d_validate_defer` owns the file-scope reject and fires first for all known inputs, so this Pass-2 guard is unreachable-by-design; it is kept live in release as a **fail-loud backstop:** should any future Phase-1 coverage gap let a `defer` reach Pass 2 at file scope, it raises a hard error rather than registering a defer whose body is silently dropped (a token-losing miscompile). Converting a potential silent-miscompile into a diagnostic is the safer failure mode and is the intended behavior. Same-block defer shadows (variable declared after a defer in the same scope) are detected in Phase 1 (`p1_check_defer_same_block_shadow`) and do not rely on `check_defer_shadow_at_exit`. Pass 2's remaining defensive `pparse_error_tok` calls in `process_declarators`, `emit_bare_orelse_impl`, `emit_orelse_action`, etc. are unreachable-by-design assertions compiled only under `PRISM_DEBUG` (see §1): they guard against Phase 1 coverage gaps, not against user input. Phase 1D detects: array orelse, struct value orelse, compound-assign bare orelse, bare orelse without assignment target, empty orelse actions (`orelse ;` / `orelse ,`, including chain tails, in bare statements and declaration initializers), storage-class and `constexpr` orelse initializers, const + variably-modified orelse (including typeof/`_Atomic` VM), multi-declarator VM splits, anonymous struct/union multi-declarator split, bracket orelse in control-statement init clauses, orelse inside `case` label expressions (both at the label's top level and inside any paren/bracket group between `case` and the terminating `:`: the group-interior scan closed a silent-passthrough hole found by the alphabet totality suite), VLA-type cast in bare orelse RHS, volatile dereference with compound-literal fallback, and preprocessor conditional spanning. The preprocessor boundary check (rejecting bare orelse assignments that span `#ifdef`/`#ifndef`/`#if`/`#elif`/`#else`/`#endif`) is the Phase 1D primary; the copy in `emit_bare_orelse_impl` is a debug assertion. This check is unreachable in CLI mode (input arrives from `cc -E` with conditionals already resolved) but reachable in Library Mode (`prism_transpile_source`), where raw un-preprocessed source preserves conditional directive tokens.
...
10. A *defer-statement* whose body is a single declaration with an `orelse` initializer (`defer T name = expr orelse fallback;`) is a constraint violation. Wrap the body in braces (`defer { T name = expr orelse fallback; }`) so the declaration is processed at statement-start within a fresh block scope.

11. A *defer-statement* whose braceless body is *any* declaration (`defer T name;`, `defer T name = init;`) is a constraint violation. The deferred-range emission re-parses the declarator against the token stream and overshoots the body's terminating `;`, duplicating following statements into the pasted cleanup. Wrap the body in braces: `defer { T name = init; }`. (Found by the combinatorial context-closure and insertion suites; see `.github/PROOFS.md`.)

12. The `defer` keyword shall appear only at statement position. A `defer` wedged into a declarator, initializer, function-argument list, array dimension, `sizeof`/`_Static_assert` operand, or any other expression context is a constraint violation (it would otherwise be consumed by statement-level emission and mangle surrounding tokens). A `defer` used as an ordinary identifier: a variable, a `struct`/`union`/`enum` tag name, a member name, or a typedef name: remains valid where the extension is disabled or the name is so bound.
...
4a. An `orelse` shall not appear inside a `case` label expression (at any parenthesis or bracket depth between `case` and the label's `:`). Case labels require integer constant expressions and provide no statement context for lowering.
...
8. In contexts where the array dimension expression may be evaluated more than once (VLA sizes, `typeof` operands), neither operand of a *bracket-orelse* shall contain side effects. Prohibited side effects include:
   ...
   - Volatile reads through indirection (`*`, `->`, `.`, `[]`): array-dimension operands only; within a `typeof` operand the duplicated expression is unevaluated unless the resulting type is variably modified (ISO C23 §6.7.2.5), so indirection reads are permitted there
   - Inline assembly (`asm`)
...
14. `orelse` shall have a left operand. An `orelse` with nothing before it in its expression: a bare `orelse` at statement start, or a declaration initializer of the form `T x = orelse fallback;`: is a constraint violation ("expected expression before 'orelse'").

15. In a *bracket-orelse* (`[dim orelse fallback]`) the fallback shall be a non-empty expression; `[dim orelse]` (empty fallback, or a fallback that is only a closing delimiter, `;`, or `,`) is a constraint violation.

16. A declaration-initializer `orelse` shall not straddle a preprocessor conditional between the initializer start and the `orelse` keyword (e.g. `int x =\n#if C\n a orelse b\n#else\n c\n#endif\n;`): the lowering would split the assignment scaffolding across branches. A conditional entirely within the fallback (after `orelse`) is permitted because both branches are plain values. (Reachable only in Library Mode; CLI input arrives preprocessed.)
...
8. **Volatile and atomic safety:** In bare-assignment forms, the truthiness test uses the value produced by the C assignment expression `(LHS = expr)`, not a re-read of `LHS`. For volatile-qualified lvalues, this guarantees exactly one write when the result is truthy, and no hidden reads. For **declaration** value-fallback `orelse` (`type name = expr orelse fb`), when the stored object is **`volatile`** or **`_Atomic`-qualified** (including typedefs whose definition used `_Atomic(...)`), Pass 2 lowers with a braced `if (!name) { name = fb; }` pattern instead of `name = name ? name : fb` so the object is not read twice (`emit_orelse_action` / `single_eval_lhs`; see §5 Pass 2).
...
4. A `register`-qualified variable-length array (VLA) without an explicit initializer and without `raw` is a constraint violation. `register` prohibits address-taking (ISO C11 §6.5.3.2¶1), making `memset` impossible, and VLAs cannot use `= {0}` syntax.
