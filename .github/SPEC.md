...
GNU extension `__auto_type` is tagged `TT_TYPE | TT_TYPEOF` so that declaration detection and goto-over-decl checking treat it identically to `typeof`/`__typeof__`. The `typeof_unqual` family (`typeof_unqual`, `__typeof_unqual__`, `__typeof_unqual`) is tagged `TT_TYPE | TT_TYPEOF`; `parse_type_specifier` detects the unqual variant via `tok->len >= 13` (all three lengths — 13, 15, 17 — exceed the longest non-unqual typeof variant `__auto_type` at 11). The parenthesized operand is **always** scanned for struct/union tags and aggregate metadata (`is_struct`, `is_union`, typedef/tag volatility inherited from *tags*) so zero-init / const-safety guards stay correct. Only **qualifier propagation from the inner token walk** is suppressed for `typeof_unqual` (`!is_unqual` gates `const`/`volatile`/`atomic` bits and volatile typedef lookups on ordinary identifiers inside the parens), since those qualifiers are stripped from the represented type by definition.
...
- `array_rank` — number of array declarator dimensions contributing to the tracked variable (0 if not an array). Stored as `uint8_t`. Counted at Phase 1D registration when `FEAT(F_BOUNDS_CHECK)` is on (and on typedef entries from `parse_typedef_declaration`) by scanning declarator `[` with `TF_OPEN`, then adding the base typedef’s `array_rank` or `TypeSpecResult.type_array_rank` for `typeof`/`_Atomic` array bases (`typedef int T[3][4]; T a;` → rank 2; `typedef int Row[5]; Row m[3];` → rank 2). When the summed rank exceeds 15, the stored value is the sentinel **`ARRAY_RANK_WRAP_ALL` (255)** instead of clamping to 15 — clamping caused the rank guard (`dim_depth >= array_rank`) to match the outermost legitimate index of a 16+ dimensional array and incorrectly disable wrapping. For sentinel ranks, `try_bounds_check_subscript` does **not** apply the peel-off early return, so every subscript level remains eligible for wrapping. For ranks 1–15, consumed as a rank guard against spurious wraps on `int *p[10]; p[i][j]` — without it the inner subscript would emit `sizeof(p[0])/sizeof(p[0][0]) == sizeof(ptr)/sizeof(int)` and trap on every valid pointer subscript.
- `array_dim_complete` — for typedef entries that describe an array type (`is_array`), whether `sizeof(T)/sizeof(T[0])` is meaningful at uses: `typedef int A[3]` → true; `typedef int A[]` (or only empty `[]` without a completing initializer) → false; chained typedefs inherit from the base array typedef. Phase 1D bounds registration uses this when the declarator has **no** `[...]` but the variable is still an array because the base type is a typedef (`extern IncompleteArray buffer` where `IncompleteArray` names an incomplete array type) — registration is skipped so Pass 2 never emits `sizeof` on an incomplete type. When the array shape comes only from `typeof(...)` / `_Atomic(...)` (e.g. `typeof(int[10]) buffer;`), completeness is recovered separately by scanning the type specifier for a non-empty `[expr]` dimension (`p1d_type_spec_has_nonempty_array_dims`).
...
- `is_param` — set for function parameter shadow entries (registered by `p1_register_param_shadows`). Used by `array_size_is_vla` to distinguish VLA parameters (which decay to pointers, making `sizeof(param)` constant) from VLA locals. For `-fbounds-check`, the same path sets `is_ptr` on the shadow when the token before the parameter name is `*` (so `uint32_t *cache` is recorded as a pointer parameter). **`array_size_is_vla`:** Inside `sizeof(...)`, binary `+`/`-` immediately **after** a decayed array-parameter identifier are treated as pointer arithmetic (`param+5`), not as unary or postfix deref — `sizeof(param+5)` remains pointer-sized and must not classify the enclosing array dim as VLA-backed.
...
The duplicate-detection guard in `typedef_add_entry` compares entry kind (`prev->is_struct_tag == (kind == TDK_STRUCT_TAG)`) in addition to scope depth and range — a `TDK_TYPEDEF` and `TDK_STRUCT_TAG` with the same name at the same scope are NOT duplicates (separate C11 namespaces). **`TDK_SHADOW` and `TDK_VLA_VAR`** use the same skip-if-duplicate-at-same-scope rule so repeated registrations (e.g. hostile macro replay) cannot lengthen `prev_index` chains.
...
**Braceless body detection:** The for-init body end is determined by `skip_one_stmt()`, an iterative helper that correctly handles all C statement forms: braced `{…}`, `if (…) stmt [else stmt]`, `for`/`while`/`switch (…) stmt`, `do stmt while (…);`, `case EXPR:` / `default:` labels, and simple `expr;`. The `case` label scanner tracks ternary depth (`?` increments, `:` decrements when depth > 0) to distinguish ternary `:` from label `:` in expressions like `case 1 ? 2 : 3:`. Without this, a ternary `:` is misidentified as the label terminator, causing the scanner to overshoot and corrupt shadow scope boundaries. This prevents if-else branches from prematurely terminating the shadow scope (e.g., `for (int T=0;…) if(c) x=T; else T*x;` — the else branch is within T's shadow scope). **Cache correctness:** The O(1) amortization cache (`skip_cache`) stores trail entries during traversal and backfills them when a statement end is reached. When unwinding an `if`-body and entering the `else` branch, the trail must be flushed (cached with the true-branch's end value) and reset before processing the else. Without this, tokens inside the true-branch are poisoned with the else-branch's end value, causing subsequent cache queries on those tokens to return the wrong statement boundary, producing false-positive CFG errors. **`if_depth` snapshot cap:** Trail snapshots are only recorded while `if_depth < SOS_IF_MAX` (512). Beyond that, missing snapshots cannot use sentinel `snap=0` (that would truncate `tn` and poison the cache); **`else`** unwinding uses `snap = tn` so the flush loop is a no-op until depth drops back below the cap.

**`do` / nested `if` snapshot buffer:** Each `do` saves the current `if_trail_snap[]` slice into a flat buffer (`do_snap_buf`, cap `SOS_SNAP_MAX`) so `while(0)` unwinding can restore trail state. If the **required** snapshot size would exceed the buffer (`do_snap_top + if_depth` at `do` entry), Prism **errors** (`error_tok`, message mentions `trail snapshot`) instead of silently dropping pushes — silent truncation could desynchronize `tn` and corrupt `skip_cache` / trail indices.

**Struct/union body — volatile / VLA field scans:** `struct_body_contains_volatile` and struct-body VLA detection must not treat a **member name** as a typedef merely because it spells like one (ISO C §6.2.3 member namespace vs ordinary identifiers). An identifier followed (after noise) by `;`, `,`, `:`, `[`, or `=` is treated as a **field name** (`struct_body_id_is_field_name`) and skips `typedef_lookup`-based volatile/VLA inference for that token.
...
**Multi-declarator VM-type split restriction:** When `process_declarators` must split a multi-declarator statement (due to the triggers above), it re-emits the type specifier for the new declaration. If the type specifier contains a variably-modified type — `typeof(expr)` or `_Atomic(type)` where any array dimension in the parenthesized specifier is variably modified — the dimension expression could be evaluated a second time at runtime by the backend compiler (C23 §6.7.2.5 — VM type specifiers are evaluated when the declaration is reached). This causes double evaluation of side effects (function calls, `++`, etc.) in VLA dimensions. Prism rejects such splits with a hard error: the user must declare each variable on a separate line. Pass 2 uses `TypeSpecResult.is_vla` for **object** VLAs in the specifier (e.g. `typeof(int[n])`) and `TypeSpecResult.type_vm` for any VM dimension scan hit (including `typeof(int(*)[n])` pointer-to-VLA forms where `is_vla` is false so const pointer-to-VLA does not false-positive the memset path). Phase 1D's `vm_type` gate mirrors that: `(type->has_typeof || type->has_atomic) && (type->is_vla || type->type_vm)`. The `_Atomic(...)` VLA detection uses the same `scan_paren_for_vla` as `typeof(...)`, including abstract pointer groups before `[n]`. This also covers the orelse `stop_comma` continuation paths (const orelse fallback, orelse action). Anonymous struct/union splits are separately rejected because re-emitting the body produces two incompatible anonymous types. **Two-Pass Invariant for split detection:** Phase 1D's `p1d_check_multi_decl_constraints` must perfectly simulate Pass 2's split logic. The `split` predicate includes three triggers: (1) `current_decl_has_orelse` — the current declarator's initializer contains an `orelse` keyword, which unconditionally forces a split via `process_init_orelse_hit` in Pass 2; (2) `any_would_memset` — a preceding declarator requires a typeof/VLA memset and the next declarator has an initializer; (3) bracket orelse on the next declarator. The `any_would_memset` condition must exactly mirror Pass 2's `needs_memset`, including `type.is_array` for typeof/`_Atomic` array types; `is_aggregate` includes `(type.is_struct || type.is_typedef || type.is_array) && !decl.is_pointer`. Previously, Phase 1D used a simplified `type.has_atomic && type.is_struct` check that missed `_Atomic` arrays and `_Atomic` typedef aggregates, causing it to approve multi-declarator statements that Pass 2 would split — violating the Two-Pass Invariant. Without the first trigger, Phase 1D would approve VM-type and anonymous struct multi-declarators where the current declarator's orelse forces a split invisible to the static analyzer, causing Pass 2 to crash mid-emission (violating the Two-Pass Invariant). The `any_would_memset` arm also checks `nd.is_vla` (next declarator has VLA dimensions), matching Pass 2's `should_split_multi_decl` which splits when `typeof_var_count > 0 && (next_decl.has_init || next_decl.is_vla)` — without the VLA check, a `typeof(int[n]) arr, buf[get_n()]` declaration would bypass Phase 1D and crash in Pass 2.

**Const orelse VM-type restriction:** `handle_const_orelse_fallback` emits the type specifier twice — once for the mutable temporary and once for the final `const` declaration. When the type is variably-modified, this forces the C compiler to evaluate VM type operands twice at runtime (C23 §6.7.2.5). Pass 2 rejects const-qualified value fallback when **`type->is_vla || decl->is_vla || type->type_vm`** — matching Phase 1D’s `vm_type` predicate `(type.has_typeof || type.has_atomic) && (type.is_vla || type.type_vm)`. Object VLAs set `is_vla`; pointer-to-VLA / abstract VM dimensions set **`type_vm`** without `is_vla` (e.g. `const typeof(int (*)[n++]) ptr = … orelse …`). Multi-declarator deferred type emission uses the same **`is_vla || type_vm`** check so splits cannot duplicate VM specifiers. The user must hoist the value to a non-const variable first (or use control-flow / non-value orelse). This intentionally rejects **`const _Atomic(int(*)[n]) p = get() orelse NULL`** alongside typeof forms where the specifier would be emitted twice.
...
8. **`in_ctrl_paren()`:** `emit_decl_init_walk` only detects noreturn when `!in_ctrl_paren()`. For `for`/`if`/`switch` parenthesized headers, `process_declarators` must not emit `__builtin_unreachable()` after an initializer `;` — that would insert a standalone statement inside the control header (`for (int x = exit(0), 0; __builtin_unreachable(); ; )`), which is invalid C.
...
15. Single declarator — multi-declarator `const int a[2] = {1,2}, b[2] = {3,4};` is excluded because promoting only one declarator to `static` while leaving the other `auto` would require splitting the declaration, which violates the principle of minimal transformation. **Mixed multi-declarator:** the same gate applies when the first declarator qualifies for promotion but a **second** declarator appears at brace depth 0 (`decl_followed_by_another_declarator` / `peek_flat_init_until_comma_sem`), e.g. `const int a[2] = {1,2}, b = f();` — auto-static must not prefix `static` for the entire declaration line.
...
6. The `[` token does not carry `P1_UNEVAL_BRACKET` — tagged by `p1_mark_uneval_brackets` for every `[` inside a parenthesized `sizeof` / `_Alignof` / `alignof` / `typeof` / `typeof_unqual` / `offsetof` / `__builtin_offsetof` operand, for every `[` in the controlling expression of `_Generic(arr[i], ...)` (per C11 §6.5.1.1p3 the controlling expression is not evaluated — the bounds trap must not fire on a type-selection expression), and for every `[` inside the parenthesized operand of `_Static_assert` / `static_assert` (the predicate must remain an integer constant expression). The operand is unevaluated, and in `offsetof` a subscript names a struct field (unrelated to any same-named local), so wrapping would be a silent false negative or a spurious trap on VLAs.
...
11. For a subscript at inner position `d` of a multi-dim access (i.e. `d` prior `[...]` layers were peeled to reach the root identifier — see *Multi-dim* below), either `te->array_rank == ARRAY_RANK_WRAP_ALL` (255), or `d < te->array_rank`. This is the *rank guard*: it prevents `int *p[10]; p[0][i]` from emitting `sizeof(p[0])/sizeof(p[0][0])` (= `sizeof(ptr)/sizeof(int)`) as the bound — which would spuriously trap on every valid pointer subscript. When `array_rank` is the overflow sentinel (true rank > 15), the guard never returns early from rank alone, so high-dimensional arrays still get wraps on every level.

**Commutative-subscript rejection.** ISO C defines `idx[arr]` as equivalent to `arr[idx]`. When the array side hides *inside* the brackets (e.g. `len[buffer]` or `idx[arr[0]]` where the element is another subscript), the array-on-the-left pattern-match fails and the subscript would be emitted raw — silently skipping the trap. Before taking the normal paths, `try_bounds_check_subscript` rejects when a **tracked array** appears as the indexed value inside the brackets while `last_emitted` is **not** a tracked array (after the same paren-stripping used elsewhere): bare `arr` before `]`; **and** postfix chains `arr[expr]…` that consume the whole bracket span up to the closing `]`. **Pointer bases** (`uint32_t *cache` in `cache[trail[i]]`) are excluded: `bounds_expr_base_is_pointer` walks the typedef chain for `is_ptr`, and parameter shadows created for `-fbounds-check` copy `*` before the declarator into `is_ptr` so nested indices into `trail` are not confused with commutative `idx[arr]`. Hard error text: `commutative subscript 'idx[arr]' bypasses -fbounds-check; rewrite as 'arr[idx]'`.

**Comma-operator subscript.** `idx[(e1, arr)]` is ISO-equivalent to `idx[arr]` but cannot be instrumented in place without rewriting the whole postfix expression; Prism hard-errors with `comma operand subscript idx[(...,arr)] must be rewritten as arr[idx]`.

**Pointer-to-array declarator rank.** `abstract_declarator_paren_is_pointer_only` treats concrete direct declarators inside `(...)` (e.g. `*p[5]` in `int (*p[5])[10]`) like abstract pointer declarators so `array_bracket_closes_ptr_to_array` recognizes the `[10]` that completes the pointer-to-array type — outer `[5]` contributes rank, inner `[10]` does not inflate `array_rank` for `p`.

**Multi-dim postfix chain.** After paren-stripping, if `name_tok` is `]` (close of a prior subscript) or resolves through parens into one, the walker follows `tok_match` back to the matching `[`, increments `dim_depth`, and continues until it reaches the root identifier. The emission uses `sizeof(arr[0]...[0])/sizeof(arr[0]...[0][0])` with `dim_depth` and `dim_depth+1` `[0]` subscripts so `arr[i][j]` / `(arr[i])[j]` / `arr[i][j][k]` all wrap every level. Rank is recorded on the shadow `TypedefEntry` (`array_rank`, `uint8_t`) at registration time by counting declarator `[` with `TF_OPEN`, **skipping** brackets that close a pointer-to-array abstract declarator (`array_bracket_closes_ptr_to_array` — e.g. the `[20]` in `int (*p[10])[20]` must not add a second rank to the variable `p`), then adding the base typedef's own `array_rank` or `type_array_rank` from the type specifier (so `typedef int T[3][4]; T a;` records rank 2 and `typedef int Row[5]; Row m[3];` records rank 2 by summing 1 + 1). If the sum exceeds 15, `array_rank` is stored as **`ARRAY_RANK_WRAP_ALL` (255)**. Chained typedefs (`typedef T1 T2; T2 a;`) inherit through the lookup at each link. Because `last_emitted` is unchanged by synthetic output tokens, the walker consults `token_pool[tok_idx(tok) - 1]` when it differs from `last_emitted` to recover the true pool predecessor.

Array variables are registered into the typedef table with `is_array = true` by a Phase 1D hook that runs only when `FEAT(F_BOUNDS_CHECK)` is on, so the feature is strictly zero-cost for users who leave it off. Registration covers block-scope, `static` block-scope, and file-scope arrays. Registration requires a **complete** outer array type at subscript use sites. **Outer dimension rule:** completeness is determined from the **first** declarator `[` only — `int a[][10]` / `extern int m[][10]` remain **incomplete** even when an inner `[10]` has a size; Prism must not register them for `sizeof(m)/sizeof(m[0])` wrapping. Every declarator-level `[]` must either contain a size expression on that **first** bracket or be completed by an initializer on the same declarator (`int g[] = {1,2,3};`). Empty first `[]` with no size and no completing initializer yields an **incomplete** array type — `sizeof(arr)` is not a constant expression at uses, so Prism must not emit `sizeof(arr)/sizeof(arr[0])` in the bounds helper. Therefore **`extern int a[];`**, **tentative `int a[];` at file scope**, and **block-scope `extern int a[];`** are NOT registered (same incomplete-array rule at every scope). **`typedef int IncompleteArray[]; extern IncompleteArray buffer;`** is also not registered: the declarator has no `[]`, but the base typedef entry has `array_dim_complete == false`. Array types that appear only inside `typeof(...)` / `_Atomic(...)` (e.g. `typeof(int[10]) buf;` with no `[` in the declarator) are recognized via `TypeSpecResult.is_array` / `type_array_rank` from `scan_paren_for_vla`; completeness for registration additionally requires at least one non-empty `[expr]` dimension in the type specifier (`p1d_type_spec_has_nonempty_array_dims`), so incomplete `typeof(int[])` forms are not registered.
...
| `extern int a[]` block or file scope, incomplete `[]` | No | Not registered — `sizeof(a)` incomplete at uses; wrapping would emit invalid sizeof |
| `typedef int IA[]; extern IA buf` (block or file scope) | No | Same rule via `TypedefEntry.array_dim_complete`; declarator has no `[]` to scan |
...
| `idx[arr]` / `idx[arr[i]]` (commutative forms) | **Rejected** | Hard error — rewrite as `arr[idx]` / normal indexed form (otherwise the trap would silently not fire) |
| `idx[(0, arr)]` (comma tail is array) | **Rejected** | Hard error — rewrite as `arr[idx]` (cannot instrument without full-expression rewrite) |
...
When `-fno-flatten-headers` is active, Prism runs `cc -E` which consumes in-file `#define` directives. The transpiled output reconstructs `#include` directives but must also re-emit the user's `#define`s that appeared before the first `#include`. `collect_source_defines` scans the original source file (raw text, not tokens) and extracts non-function-like `#define NAME VALUE` directives, both unconditional and conditional. These are re-emitted as `#ifndef NAME` / `#define NAME VALUE` / `#endif` guards by `emit_consumed_defines` before any `#include` directives. **Prism `raw` in macro bodies:** Pass 2 strips `raw` from the token stream, but scraped `#define` text is copied from the raw source; without filtering, a define like `#define T raw int` would re-emit `raw` and break the backend. Before building each stored `NAME=VALUE` string, `strip_prism_raw_from_macro_value` removes whole-token `raw` markers from the replacement text (word-boundary match; identifiers like `draw` are untouched). **String/character literals:** The scanner skips `"'`…`'` / `"…"` spans with `\` escape awareness so substring `raw` inside a string (e.g. `#define MSG "failed raw data"`) is never stripped.
