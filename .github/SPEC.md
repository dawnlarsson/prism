...
**GNU nested function gating:** GCC nested function definitions inside an outer function are passed through verbatim as plain GNU C *unless* the outer function actually uses `defer`. A nested function inside a defer-using outer function is rejected because Prism's defer transform conflicts with GCC nested-function semantics (the nested function captures the outer's locals via a trampoline, but Prism's defer cleanup runs in a synthesised epilogue that the trampoline cannot see). The check at `p1d_probe_declaration` walks the entire outer function body forward from `func_meta[cur_func].body_open` to its matching `}` looking for any `defer` keyword via `is_defer_kw`; only when found is the nested-function rejection raised. Plain GNU C nested-function idioms — including statement-expression nested function definitions like `int (*fp)(int) = ({ int g(int x) { return x + 1; } g; });` — are accepted unchanged in defer-free outer functions. The forward scan covers `defer` keywords appearing both before and after the nested function definition in source order.
