...
**const VLA rejection:** `const`-qualified VLA declarations requiring `typeof`/VLA memset are rejected with a hard error — modifying a `const`-defined object is undefined behavior (ISO C11 §6.7.3p6). The check examines `type->has_const`, `decl.is_const`, and const-qualified typedefs in the type specifier range. Non-VLA `const typeof` (e.g., `typeof(const int) a;`) is allowed because compilers optimize the memset to a compile-time initializer.
