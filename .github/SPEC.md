...
| `scope_tree[]` | `ARENA_ENSURE_CAP` | `tokenizer_teardown(false)` |
| `func_meta[]` | `ARENA_ENSURE_CAP` | `tokenizer_teardown(false)` |
| `p1_entries[]` | `ARENA_ENSURE_CAP` | `tokenizer_teardown(false)` |
| `p1_shadow_entries[]` | `ARENA_ENSURE_CAP` | `tokenizer_teardown(false)` |
| `p1_shadow_map.buckets` | `arena_alloc` | `tokenizer_teardown(false)` |
| `typeof_vars[]` | `ARENA_ENSURE_CAP` (ctx, reused) | `tokenizer_teardown(false)` |
| `bracket_oe_ids[]` | `ARENA_ENSURE_CAP` (ctx, reused) | `tokenizer_teardown(false)` |

All arena-allocated structures are automatically reclaimed by `prism_reset()` → `tokenizer_teardown(false)` on any `error_tok()` that `longjmp`s out. `arena_alloc_uninit` has an overflow guard: `size > SIZE_MAX - (ARENA_ALIGN - 1)` is rejected before the alignment addition, preventing wrap-around to a tiny allocation.
...
- `is_vla`, `is_void`, `is_const`, `is_volatile`, `is_ptr`, `is_array`, `is_aggregate`, `has_volatile_member`, `is_atomic`: type property flags (`is_atomic` is set when the typedef's type specifier used `_Atomic(...)`: see `TDF_ATOMIC` / `typedef_apply_tdf_flags`)
- `is_shadow`, `is_enum_const`, `is_vla_var`, `is_struct_tag`: entry kind flags. `is_struct_tag` entries (`TDK_STRUCT_TAG`) are unconditionally registered for all struct/union tags with bodies, including clean structs with no VLA or volatile members. This ensures inner-scope struct redefinitions correctly shadow outer tags (C11 §6.2.1p4), preventing `tag_lookup` from falling through to stale outer-scope VLA/volatile metadata and causing false CFG verifier errors. Registration occurs at two sites: Phase 1A's `TT_SUE` handler (bare struct defs) and `typedef_declaration` (typedef-wrapped struct defs). Clean entries have `is_vla=false, has_volatile_member=false`. They do NOT receive `P1_IS_TYPEDEF` annotation, preserving `struct Foo` parsing (the tag is not an ordinary typedef name per ISO C11 §6.2.3). Tag name extraction in Phase 1D uses `skip_noise()` to skip GNU `__attribute__` and C23 `[[...]]` attributes between the struct/union keyword and the tag name, and skips `TT_QUALIFIER` tokens
- `is_func`: set when the typedef resolves to a function type, and also on `TDK_SHADOW` entries that represent block-scope function declarations. This suppresses zero-init memset on function types and lets `typeof(name)` distinguish a local function declaration from an ordinary local variable/parameter shadow. Typedef detection works for both `typedef int FuncType(int)` (parse_declarator returns `end=NULL`, check for `(` after name), `typedef int (FuncType)(int)` (parse_declarator sets `is_func_ptr=true` without `paren_pointer`: function type, not function pointer), and parenthesized function declarators such as `typedef int (*Getter(void))[10]` (`decl.is_func_decl`). **Chained typedef propagation:** `typedef_declaration` propagates `is_func` through typedef chains via `base_is_func`: when the base type specifier contains a function typedef (`is_func_typedef`), the derived typedef inherits `is_func = true` (guarded by `!decl.is_pointer && !decl.is_array && !decl.is_func_ptr`). Without this, `typedef func_t alias; alias f;` would lose the function-type property and Prism would emit `= {0}` or `memset` on a function symbol: a fatal constraint violation (ISO C11 §6.5.3.4p1 forbids `sizeof` on function types).
...
A single lookup returns a bitmask: `TDF_TYPEDEF`, `TDF_VLA`, `TDF_VOID`, `TDF_ENUM_CONST`, `TDF_CONST`, `TDF_PTR`, `TDF_ARRAY`, `TDF_AGGREGATE`, `TDF_VOLATILE`, `TDF_FUNC`, `TDF_PARAM`, `TDF_HAS_VOL_MEMBER`, `TDF_UNION`, `TDF_ATOMIC`. Convenience macros (`is_known_typedef`, `is_vla_typedef`, `is_volatile_typedef`, `has_volatile_member_typedef`, etc.) test individual bits.
...
For every variable declaration at every depth, if the declared name collides with a typedef, a `P1ShadowEntry` is recorded via `p1_register_shadow`. Shadows also create a `TDK_SHADOW` entry in the typedef table itself (with `is_shadow = true`, `scope_open_idx`, `scope_close_idx`), so that `typedef_lookup` returns the shadow entry when the token is within the shadow's scope range. Shadow registration triggers when the declared name matches any of: `TT_DEFER`, `TT_ORELSE` (keyword shadowing), `TT_NORETURN_FN`, `TT_SPECIAL_FN` (noreturn/setjmp function name shadowing), or an existing typedef/enum-constant/function-prototype entry in the typedef table (type shadowing). Block-scope function declarations (`decl.is_func_decl`) also register a `TDK_SHADOW` entry marked `is_func = true`; this preserves local prototype visibility for `typeof(local_function_name)` while still allowing ordinary object shadows to hide function/prototype names. **Volatile force-registration:** declarations whose type is volatile-qualified (directly via `TT_VOLATILE`, transitively via a `TDF_VOLATILE` / `TDF_HAS_VOL_MEMBER` typedef, or whose struct/union tag carries `has_volatile_member`) force a shadow entry even when no name-collision exists, so Pass 2's `emit_token_range_orelse` sees the volatile qualification via a normal `typedef_lookup` and rejects compound-literal fallbacks uniformly. This covers ordinary block-scope locals (main decl loop), function parameters (`p1_register_param_shadows`), and for-/if-/switch-init declarators (`p1_scan_init_shadows`).
...
**Invalid context rejection (Phase 1):** Orelse inside `struct`/`union` bodies, inside `enum` bodies (enum constants must be compile-time integer constants), and inside `_BitInt(N)` / `_Alignas(N)` (compile-time constant expression contexts) is caught and errored. The `_BitInt`/`_Alignas` check fires inside `parse_type_specifier` (called from Phase 1D prescan).
...
- the next token is **not** already `TF_AT_BOL`.

Suppressed **inside aggregate member lists**: `in_struct_body()` (scope stack marks `= { … }` initializers and keyword-headed aggregates opened via `handle_open_brace`), and `ctx->aggregate_member_nest`, incremented when emitting `{` whose **predecessor** carries `TT_SUE` (`struct`/`union`/`enum`), decremented on `}`, so anonymous SUE bodies inside balanced walkers (e.g. `typeof_unqual(union { int a; long b; })`) are not split.

`handle_open_brace` tags keyword-headed aggregates using the `{` token’s **pre-open** predecessor (`tok_before_brace`), not `last_emitted` after emitting `{` (that token is punctuation with `TT_STRUCTURAL`, not `TT_SUE`).

### `orelse` value fallback on volatile / `_Atomic` declarations

Declaration-initializer value fallback (`T x = expr orelse fb`) must not lower to `x = x ? x : fb` when **`volatile`** or **`_Atomic`-qualified** storage would make the double read of `x` observable. `emit_orelse_action` uses `if (!x) { x = fb; }` instead (`single_eval_lhs`). `_Atomic` in the type specifier sets `TypeSpecResult.has_atomic`; typedefs defined with `_Atomic(...)` set `TypedefEntry.is_atomic`, exported via `TDF_ATOMIC` / `typedef_apply_tdf_flags`, so names like `typedef _Atomic(int*) atomic_ip_t` inherit the same behavior.
...
Both HashMaps are populated during Phase 1D by `defer_body_populate_captures(body, body_end, out, body_captures_map)`, a single-pass O(M) scanner that computes the exact set of captured (free) variables for each defer body. A captured variable is one referenced in the defer body but not locally declared within it at the reference's scope depth or deeper. The scanner maintains per-name state in a local HashMap (`name → decl_depth`), a scope-exit stack for resetting local declarations when `}` closes a brace depth, and for-init scope tracking for C99 `for(TYPE name = ...)` declarations. When an inner scope redeclares an identifier, the stack records `had_binding` and `prev_val` **before** overwriting the slot; popping the inner depth restores the previous binding via `hashmap_put` or `hashmap_remove` (tombstones in `parse.c`), so outer declarations are never confused with captures after the inner block ends.

**`scan_paren_for_vla` / `typeof`:** `scan_paren_for_vla` registers enum constants defined inside `typeof(...)` / `_Atomic(...)` argument lists via `enum_constants` when an `enum { ... }` body is encountered, so downstream bounds checking and typedef shadow logic see ghost enumerator names. The `parse_type_specifier` `TT_TYPEOF` qualifier scan treats inline anonymous struct bodies (`struct { ... }` after `struct`/`union`) as full struct bodies for `struct_body_contains_volatile`.
...
**Limitation:** Does not support struct/union values (no whole-struct zero check). Struct/union pointers work. Rejected in Phase 1 for both declaration form (`T x = e orelse …`) and bare assignment (`x = e orelse …`, including `*p = get() orelse …` when `get` returns a struct/union by value). Typedeffed structs in declaration form are rejected when `parse_type_specifier` sets `is_struct` (`TT_SUE` or `TDF_AGGREGATE`). `typeof(struct S *)` / `_Atomic(struct S *)` are treated as pointers (outer `*` clears the struct-value mark). Opaque `typeof(expression)`: where the expression is a variable name (e.g. `typeof(my_var)`) or a function call (e.g. `typeof(make())`) whose type is an aggregate: cannot always be detected at the token level. The backend compiler reports a clear error (`used type ... where arithmetic type is required` or `wrong type argument to unary '!'`).
...
**Keyword shadow disambiguation (annotation model):** When the identifier `orelse` is shadowed by a variable, enum constant, or typedef name (`TT_ORELSE` tag co-exists with a typedef-table entry), positional context decides keyword vs identifier. The decision is made **once, in Phase 1**, and baked into the `P1_IS_ORELSE_KW` annotation bit; Pass 2 trusts the bit and never re-derives the answer.

- `orelse_shadow_is_kw(prev)`: the core positional predicate. Given the token *before* the ambiguous `orelse`, returns true when context indicates keyword usage: preceded by an expression-ending token (`)`, `]`, identifiers, numbers, `}`, postfix `++`/`--`). Returns false for type keywords, qualifiers, storage classes, struct/union/enum, `typeof`/`_BitInt`/`_Alignas`, `inline`, attributes, and all non-expression-ending tokens. The `TT_MEMBER` check (`.`/`->` before `orelse`) is performed by the callers. Wrapped by `orelse_kw_at_bare` / `orelse_kw_at_shadow` (which add the typedef-table lookup, known-function-call, and label/goto-target guards).
...
**Annotation consumers:** Post-annotation scanners identify orelse keywords by testing `P1_IS_ORELSE_KW` (plus `P1_OE_BRACKET` / `P1_OE_DECL_INIT` where the form matters) instead of re-running positional checks: `find_bare_orelse`, `orelse_has_chain`, `scan_bracket_orelse`, `scan_decl_orelse`, `emit_token_range_orelse`, `try_bracket_orelse` / `try_typeof_orelse` (which test `P1_OE_BRACKET` on the opening `[` / typeof `(` directly: no content rescan), and `emit_decl_init_walk`. `find_bare_orelse` and `orelse_has_chain` retain a live positional path gated on `!p1_typedef_annotated`, because Phase 1 itself calls them during discovery, before annotation bits exist. `check_orelse_in_parens` still scans positionally (it validates contexts where orelse is *forbidden*, so no annotation exists); all its call sites use `(FEAT(F_ORELSE) || FEAT(F_DEFER))` so defer-in-paren validation fires even when `-fno-orelse` is set.
...
**Declaration prefix handling:** `try_zero_init_decl` skips storage class specifiers (`static`, `extern`, etc.), `inline`, and `__extension__` to reach the actual type keyword. The P1_IS_DECL fast gate probes past these prefixes (`TT_STORAGE | TT_INLINE | TT_SKIP_DECL`) to find the Phase 1D annotation on the type-start token, ensuring `__extension__ int x;` and `__extension__ struct S s;` receive zero-initialization correctly. Phase 1D's prescan also skips `__extension__` (tagged `TT_INLINE`) at statement start before type parsing. **For-init prefix handling:** `p1_scan_init_shadows` places the `P1_IS_DECL` annotation on the type-start token, not the first token of the declaration. When `__extension__` or storage-class specifiers precede the type keyword in a for-init declaration (e.g., `for (__extension__ int i = 0; ...)`), the annotation placement loop skips `TT_STORAGE | TT_INLINE` tokens before annotating. Without this skip, `P1_IS_DECL` would land on the `__extension__` token, and Pass 2's fast gate (which probes past `TT_STORAGE | TT_INLINE | TT_SKIP_DECL` to find the annotation) would never see it: silently dropping zero-initialization for the for-init variable. Similarly, `p1_scan_init_shadows` skips `TT_INLINE`-tagged tokens before checking the `TT_TYPEDEF` tag dispatch, so that `for (__extension__ typedef int T; ...)` correctly routes to `typedef_declaration` instead of falling through to the variable declaration path.
...
2. After the matching `)`, only closing `)` (casts / grouping) or a trailing comma-expression may intervene before the statement `;`. So `(void)(0, die());`, `die(), 1;`, and `foo(die());` inject; `sizeof(die())`, short-circuit `x && die();` / `x || die();`, and control-paren sites (`if (die())`) do not. Unevaluated operands are excluded via `c_token_is_in_unevaluated_operand`
...
3. Explicit `const` qualifier on the type (`c_decl_has_explicit_const`: `type->has_const`, declarator `const`, or const typedef) — not `has_effective_const_qual`, which also treats bare `typeof` as const for orelse temps and would wrongly auto-static mutable `typeof(int[N])` arrays
...
- **Unevaluated-operand tagging:** `c_parse_mark_unevaluated_brackets` sets `P1_UNEVAL_BRACKET = 1 << 10` on every `[` inside a parenthesized `sizeof` / `_Alignof` / `alignof` / `typeof` / `__typeof` / `offsetof` / `__builtin_offsetof` operand, the `_Generic` controlling expression, the `_Static_assert` / `static_assert` predicate, and any static-storage initializer (file-scope depth-0 `=`, plus block-scope declarations carrying `static` / `extern` / `_Thread_local` / `constexpr`). Runs once per TU, gated on `F_BOUNDS_CHECK`. File-scope initializers are detected by tracking curly-brace depth: every `=` at depth 0 opens an initializer that stays open until the matching `;`, tagging every `[` in between regardless of nesting depth. Block-scope static-storage uses a pending flag from the storage/`constexpr` keyword cleared at `;` or when entering a non-initializer `{` body. Recognition for the other cases uses `TF_SIZEOF` (sizeof/alignof/_Alignof/offsetof), `TT_TYPEOF` (typeof family), `TT_GENERIC`, lexeme match on `_Static_assert` / `static_assert` / `__builtin_offsetof`.
...
In `PRISM_LIB_MODE`, `error_tok` triggers `longjmp(ctx->error_jmp)` instead of `exit(1)`. All arena-allocated structures are reclaimed by `tokenizer_teardown(false)`. Token annotations (`ann` field) survive arena resets (same lifecycle as token pool).
...
**Cross-feature invariant:** Feature flags disable their own transformations but must never suppress error checking for other enabled features. Validation that spans multiple features uses disjunctive gates (e.g. `FEAT(F_ORELSE) || FEAT(F_DEFER)` for `check_orelse_in_parens` call sites). The `defer`-in-parens check inside `check_orelse_in_parens` is internally gated by `FEAT(F_DEFER)`, while `orelse`-in-parens checking requires no gate (it is trivially safe when orelse is disabled: the keyword is parsed as an identifier, so `TT_ORELSE` is never set).
...
**`CC` / `--prism-cc=` paths with spaces.** Unquoted executable paths that exist on disk are kept as a single argv entry (and used whole for clang/gcc detection); quoted forms still tokenize via the existing splitter.
...
**Implementation note (post-1.1.4 fix).** `main()` aliases `Cli.cc_args` into `ctx->extra_compiler_flags` *before* `compile_sources` invokes `collect_link_pragmas`, but the pragma scanner may grow `cc_args` past its initial 16-slot cap and `realloc` the underlying buffer to a new address. Pre-fix this left `ctx->extra_compiler_flags` dangling, and the next `preprocess_with_cc` → `build_pp_argv` walk segfaulted (typically observed with ≥ ~7 macOS pragma tokens in a single source, or ≥ ~17 `*`/Linux/Windows tokens, where the first geometric realloc relocates). `compile_sources` now refreshes the alias immediately after `collect_link_pragmas` returns. The `-fno-link-pragma` flag bypasses the scanner entirely and avoids the regrowth path.
...
- Register ghost enums during emit (`emit_tok` does not call `enum_constants`)
...
6. **Arena safety:** All arena-allocated Pass 1 structures are reclaimed on `longjmp` error recovery. No dangling pointers after `tokenizer_teardown(false)`.
7. **Termination watchdogs (PRISM_DEBUG):** The Phase 1 prescan and Pass 2 main walks carry debug-only progress watchdogs (budget `256·token_count + 65536` outer iterations). A cursor-stall (non-termination) bug trips a hard `error_tok` diagnostic instead of hanging the build. Release builds compile the counters out entirely.
