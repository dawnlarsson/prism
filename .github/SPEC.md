...
- `is_orelse_keyword(tok)` — unified entry point for Pass 2 **emission contexts only**: returns true if `tok` is an `orelse` keyword. First checks `TT_ORELSE` tag, then rejects `TT_MEMBER` predecessor (`last_emitted`). For **all** typedef-table entries (both real typedefs and shadows), delegates to `orelse_shadow_is_kw(last_emitted)` for positional disambiguation. Tokens with no typedef entry are treated as unambiguous keywords. **Must not be called from scan contexts** (forward-scanning loops that don't emit tokens), where `last_emitted` is stale and points to a token from a previous emission — not the predecessor of the token being scanned.
...
- `check_orelse_in_parens` — validates orelse inside `(...)`, tracks `pi` (previous inner), uses `!te || orelse_shadow_is_kw(pi)`. Also validates `defer` inside parens (gated by `FEAT(F_DEFER)`). All call sites use `(FEAT(F_ORELSE) || FEAT(F_DEFER))` to ensure defer-in-paren validation fires even when `-fno-orelse` is set.
...
- `orelse_has_chain` — forward scan for chained orelse in bare expressions, tracks `prev` at depth 0, uses inline `TT_ORELSE` + `TT_MEMBER` + `typedef_lookup` + `orelse_shadow_is_kw(prev)` check.
- `emit_bare_orelse_impl` (lhs_has_indirection chain scanner) — scans for next orelse in chained temp-based bare fallback, tracks `sprev` through balanced groups, uses inline positional check.
- `try_zero_init_decl` (stmt-expr lookahead) — probes token after `({...})` statement expression for orelse, uses `se_close` (the `)` of the stmt-expr) as predecessor for inline positional check.
- `bracket_scan_has_orelse` — scans bracket `[...]` contents for orelse, tracks `prev` at depth 0, uses inline positional check.
- `try_typeof_orelse` — scans for `typeof(... orelse ...)` pattern, tracks `prev` through the typeof paren contents, uses inline positional check.

All sites also check `TT_MEMBER` on the predecessor where applicable (either inline or via `is_orelse_keyword`'s built-in check). The remaining `is_orelse_keyword` call sites are all in emission contexts where `last_emitted` is valid: `emit_expr_to_stop` (emission loop), `emit_statements` stray orelse error (main emit loop), `emit_orelse_fallback_value` (emission), `emit_decl_init_walk` (emission), `emit_bare_orelse_impl` inner orelse detection (tokens emitted via `emit_tok_checked`), `emit_bare_orelse_impl` first-token error check (called from emission context with valid `last_emitted`), and the main loop catch-all stray orelse error.
...
| `parse.c` | Tokenizer, arena allocator, HashMap, `fast_hash`, `error_tok`/`warn_tok`, type/declaration parsing (`parse_type_specifier`, `parse_declarator`, `parse_typedef_declaration`, `parse_enum_constants`), statement skipping (`skip_one_stmt`, `skip_to_semicolon`), defer validation (`validate_defer_statement`), scope utilities (`scope_block_exits`, `scope_is_ancestor_or_self`), noreturn detection (`try_detect_noreturn_call`), VLA analysis (`array_size_is_vla`, `is_array_bracket_predecessor`, `walk_back_past_noise`), raw/const helpers (`is_raw_declaration_context`, `has_effective_const_qual`) |
