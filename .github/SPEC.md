...
- `array_rank` — number of declarator `[...]` dimensions (0 if not an array, capped at 15). Set at Phase 1D registration when `FEAT(F_BOUNDS_CHECK)` is on, and also on typedef entries so ranks propagate through typedef chains (`typedef int T[3][4]; T a;` → rank 2; `typedef int Row[5]; Row m[3];` → rank 2 via declarator+base sum; `typedef T1 T2; T2 a;` inherits through the chain). Consumed by `try_bounds_check_subscript` as a rank guard against spurious wraps on `int *p[10]; p[i][j]` — without it the inner subscript would emit `sizeof(p[0])/sizeof(p[0][0]) == sizeof(ptr)/sizeof(int)` and trap on every valid pointer subscript.
