...
**Version:** 1.0.7
**Status:** Implemented — every item in this document corresponds to behavior that exists in the codebase and is exercised by the test suite (4821+ tests + self-host stage1==stage2).
...
Note: Prism is thoroughly empirically tested (self-hosting and 4821+ test cases) but **is not formally verified.** It is designed to compile standard-compliant code, but **may not catch every obscure constraint violation** defined by the ISO C standard.
...
**Attribute control-flow rejection:** After `skip_noise()` advances past GNU `__attribute__((...))`, C23 `[[...]]`, or `_Pragma(...)` blocks, the prescan scans the skipped token range for control-flow keywords (`goto`, `return`, `break`, `continue`, `defer`). If found, a hard error is emitted. Without this, a `goto` inside a statement expression in an attribute argument (e.g., `__attribute__((aligned( ({ goto L; 8; }) )))`) would be invisible to the CFG verifier — no `P1K_GOTO` entry would be created, and the jump-over-VLA safety check would not fire. Pass 2's `emit_range_ex` correctly routes `({...})` through `walk_balanced` → `emit_statements(EMIT_NORMAL)`, which handles defer cleanup at the goto site; however, Phase 1D's blindness would leave the static CFG graph incomplete. The `defer` check uses `typedef_lookup` to avoid rejecting variables named `defer` (e.g., `typedef int defer;`).
...
`emit_deferred_range` handles defer bodies by delegating to `emit_statements(start, end, EMIT_DEFER_BODY)`, which provides bare orelse, raw stripping, zero-init, control-flow keyword tracking, and SUE body handling within deferred code.
...
- Phase 1D typeof rejection — orelse inside `typeof(...)`, tracks `prev_typeof`, uses `!te_s || orelse_shadow_is_kw(prev_typeof)`.
...
**Defer body:** `raw` is also stripped inside deferred code (`emit_deferred_range` → `emit_statements(EMIT_DEFER_BODY)`).
