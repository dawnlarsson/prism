...
**Side-effect protection:** Bracket orelse in VLA/typeof contexts rejects expressions with side effects (`++`, `--`, `=`, volatile reads, function calls) to prevent double evaluation. Function-call detection recognizes both `ident(` and `)(`  (parenthesized call) patterns. Bare-assignment orelse with value fallback additionally rejects VLA-type casts in the RHS via `reject_bare_orelse_vla_cast` (Phase 1D primary, Pass 2 defense-in-depth).
