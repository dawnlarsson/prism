...
static void test_bounds_check_unevaluated_operands(void) {
	...
	// File-scope initializer (static storage): an initializer must be a
	// constant expression (C99 §6.7.8p4). Wrapping `arr[0]` in
	// __prism_bchk(...) turns it into a non-constant function call and
	// gcc/clang reject with "initializer element is not constant".
	// Regression: openssl crypto/bn/bn_nist.c under `prism -fno-safety`.
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		PrismResult r = prism_transpile_source(
		    "static const unsigned long a[][3] = {{1,2,3},{4,5,6}};\n"
		    "struct S { const unsigned long *p; };\n"
		    "static const struct S s = { (const unsigned long *)a[0] };\n"
		    "int main(void){return (int)s.p[0];}\n",
		    "bc_static_init.c", f);
		CHECK_EQ(r.status, PRISM_OK, "bc-static-init: transpiles");
		if (r.output) {
			CHECK(strstr(r.output, "a[0]") != NULL,
			      "bc-static-init: subscript in file-scope initializer not wrapped");
			CHECK(strstr(r.output, "a[__prism_bchk") == NULL,
			      "bc-static-init: no wrap inside file-scope initializer");
		}
		prism_free(&r);
	}

	// File-scope initializer with designators and nested braces — every
	// `[` inside the initializer must remain constant, even at inner
	// brace depth (compound literals, struct-in-array init, etc.).
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		PrismResult r = prism_transpile_source(
		    ...
		    "static const struct K k[2] = {\n"
		    "  { .a = tbl[0], .b = tbl[1] },\n"
		    "  { .a = tbl[2], .b = tbl[0] },\n"
		    "};\n"
		    "int main(void){return k[0].a;}\n",
		    "bc_static_init_designator.c", f);
		CHECK_EQ(r.status, PRISM_OK, "bc-static-init-desig: transpiles");
		if (r.output)
			CHECK(strstr(r.output, "tbl[__prism_bchk") == NULL,
			      "bc-static-init-desig: no wrap inside nested initializer");
		prism_free(&r);
	}
}
...
static void test_bounds_check_commutative(void) {
	...
	// Tracked array properly subscripted inside an index expression:
	// `ptr[arr[0].field >> 1]` — `arr` is bare-NEVER, it's subscripted.
	// This is NOT a commutative bypass; `arr[0]` has its own inner bounds
	// check, and the outer LHS is a pointer (untracked). Must transpile.
	// Regression: openssl curve448.c `wnaf_base[control_pre[0].addend >> 1]`
	// where wnaf_base is `const niels_t *` and control_pre is a local array.
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		PrismResult r = prism_transpile_source(
		    "struct S { int addend; };\n"
		    "extern const int *wnaf_base;\n"
		    "int f(void){ struct S control_pre[4]={0};\n"
		    "  return wnaf_base[control_pre[0].addend >> 1]; }\n",
		    "bc_comm_idx_subscripted.c", f);
		CHECK_EQ(r.status, PRISM_OK,
			 "bc-comm-idx-subscripted: tracked-array subscripted in index is not a bypass");
		if (r.output)
			CHECK(strstr(r.output, "control_pre[__prism_bchk") != NULL,
			      "bc-comm-idx-subscripted: inner subscript still wrapped");
		prism_free(&r);
	}

	// Multiple tracked arrays in index, all subscripted — still fine.
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		PrismResult r = prism_transpile_source(
		    "int f(int *ptr){ int a[4]={0}, b[4]={0};\n"
		    "  return ptr[a[0] + b[1]]; }\n",
		    "bc_comm_two_subscripted.c", f);
		CHECK_EQ(r.status, PRISM_OK,
			 "bc-comm-two-subscripted: both subscripted, no bypass");
		prism_free(&r);
	}

	// Mix: one subscripted, one bare — the bare one is still a bypass.
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		PrismResult r = prism_transpile_source(
		    "int f(int *ptr){ int a[4]={0}, b[4]={0};\n"
		    "  return ptr[a[0] + (long)b]; }\n",
		    "bc_comm_mixed_bare.c", f);
		/* `b` appears bare (as a decayed pointer cast to long) — still
		 * a potential bypass to diagnose. */
		CHECK(r.status != PRISM_OK,
		      "bc-comm-mixed-bare: bare array ident in index still rejected");
		prism_free(&r);
	}

	// Struct-field declarator with NELEM(arr)*2 in the array dimension.
	// The dim expression uses `sizeof(arr)/sizeof((arr)[0])` — `arr`
	// appears bare inside sizeof's unevaluated operand, which is a
	// type-query, not a value use. Must NOT be flagged as commutative.
	// Regression: openssl ssl/t1_lib.c `sigalgs[TLS_MAX_SIGALGCNT]`.
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		PrismResult r = prism_transpile_source(
		    "typedef struct { int x; } L;\n"
		    "static const L tbl[] = {{1},{2},{3}};\n"
		    "typedef struct {\n"
		    "  unsigned short sigalgs[((sizeof(tbl)/sizeof((tbl)[0]))*2)];\n"
		    "} S;\n"
		    "int main(void){S s={0};(void)s;return 0;}\n",
		    "bc_comm_sizeof_nelem.c", f);
		CHECK_EQ(r.status, PRISM_OK,
			 "bc-comm-sizeof-nelem: NELEM pattern in struct-field dim not flagged");
		prism_free(&r);
	}

	// Member-LHS subscript with tracked array in index, subscripted:
	// `ptr->field[arr[0]]` — `arr` is subscripted to produce an int, and
	// the outer LHS is a member whose field type we cannot resolve. Must
	// NOT be flagged as commutative. Regression: bash execute_cmd.c
	// `fd_bitmap->bitmap[fildes[0]]`.
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		PrismResult r = prism_transpile_source(
		    "typedef struct { int *bitmap; } BM;\n"
		    "void f(BM *fd){ int fildes[2]; fildes[0]=0;\n"
		    "  fd->bitmap[fildes[0]] = 1; }\n",
		    "bc_comm_member_subscripted.c", f);
		CHECK_EQ(r.status, PRISM_OK,
			 "bc-comm-member-subscripted: member LHS with subscripted tracked array in index not flagged");
		if (r.output)
			CHECK(strstr(r.output, "fildes[__prism_bchk") != NULL,
			      "bc-comm-member-subscripted: inner subscript still wrapped");
		prism_free(&r);
	}

	// -fno-safety must downgrade commutative-bypass hard-error to a
	// warning and fall through to raw emit. Regression: bash parse.y
	// `arith_for_lineno = compoundcmd_top[compoundcmd_lineno].lineno;`
	// (genuine ISO `idx[arr]` form; user opted out of safety).
	{
		PrismFeatures f = prism_defaults();
		f.bounds_check = true;
		f.warn_safety = true;
		PrismResult r = prism_transpile_source(
		    "struct ti { int lineno; };\n"
		    "static struct ti arr[10];\n"
		    "static int idx = -1;\n"
		    "int g(void){ return idx[arr].lineno; }\n",
		    "bc_comm_warn_downgrade.c", f);
		CHECK_EQ(r.status, PRISM_OK,
			 "bc-comm-warn-downgrade: -fno-safety must not hard-error on idx[arr]");
		if (r.output)
			CHECK(strstr(r.output, "idx[arr]") != NULL,
			      "bc-comm-warn-downgrade: falls through to raw emit");
		prism_free(&r);
	}
}
